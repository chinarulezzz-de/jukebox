...
BEGIN {
    {
        ...
        system(
            q(GNOME_DISABLE_CRASH_DIALOG=1 perl -e 'use Gtk2 "-init"; use Gtk2::MozEmbed;$w=Gtk2::Window->new;my $e=Gtk2::MozEmbed->new; $w->add($e);$w->show_all; exit 0')
...
sub new_embed {
    ...
    $embed->signal_connect(open_uri     => \&about_to_load_cb)
      ;    #called before loading a new uri, must return false
    return $embed;
}
...
/* } */	";
    ...
    open my $fh, '>',
      join(::SLASH, $::HomeDir, 'mozilla_profile', 'chrome', 'userContent.css')
      or return;
    print $fh $content;
    close $fh;
}
    ...
    $embed->signal_connect(
        navigation_policy_decision_requested => sub {
            if ($_[3]->get_button == 2)    #middle-click on a link
            {
                ...
                $nb->newtab('PluginWebPage', 1, {url => $_[2]->get_uri})
                  if $nb;    #open the link in a new tab
                return 1 if $nb;
            }
            return 0;
        }
...
our %Predefined = (
    ...
    lastfm =>
      {tabtitle => 'last.fm', baseurl => 'http://www.last.fm/music/%a',},
    discogs =>
      {tabtitle => 'discogs', baseurl => 'http://www.discogs.com/artist/%a',},
    youtube => {
        tabtitle => 'youtube',
    ...
    rateyourmusic => {
        ...
        baseurl =>
          'http://rateyourmusic.com/search?searchterm=%a&searchtype=a',
    },
);
...
::SetDefaultOptions(OPT,
    StrippedWiki => 1,
    Custom => {map { $_ => {%{$Predefined{$_}}} } qw/lastfm amgartist youtube/}
);
UpdateBackend();
...
sub UpdateCustom {
    ...
    if (!defined $id)    # new custom page
    {
        ...
        $id = 'custom' if $id eq '';
        ::IncSuffix($id)
          while $Widgets{'PluginWebCustom_' . $id . '_'};    #find a new name
    }
    elsif ($active) {
    ...
    my $name =
        'PluginWebCustom_'
      . $id
      . '_';  #'_' is appended because gmb widget names cannot end with numbers
    $Widgets{$name} = \%widget;
...
sub new {
    ...
    my $back  = $self->{BBack} =
      Gtk2::ToolButton->new_from_stock('gtk-go-back');
    my $next = $self->{BNext} =
    ...
    $toolbar->insert($_, -1)
      for $back, $next, $stop, $follow, $open, $entryitem, $self->addtoolbar;
    ...
    $back->signal_connect(
        clicked => sub { ::find_ancestor($_[0], __PACKAGE__)->go_back });
    $next->signal_connect(
        clicked => sub { ::find_ancestor($_[0], __PACKAGE__)->go_forward });
    $stop->signal_connect(
        clicked => sub { ::find_ancestor($_[0], __PACKAGE__)->stop_load });
    $open->signal_connect(
        clicked => sub {
...
#my $combo=::NewPrefCombo(OPT.'Site',[sort keys %sites],'site : ',sub {$ID=undef;&Changed;});
    ...
    $vbox->pack_start($_, ::FALSE, ::FALSE, 1)
      for $radio_wk, $radio_moz, Gtk2::VSeparator->new, $check, $Bopen;
    $vbox->pack_start(GMB::Plugin::WebContext::Custom::Edition->new,
        ::TRUE, ::TRUE, 8);
...
sub set_stripped_wiki {
    GMB::Plugin::WebContext::MozEmbed::set_stripped_wiki(
        $::Options{OPT . 'StrippedWiki'});
}    #FIXME
...
my %sites = (
    lyrc =>
      ['lyrc', 'http://lyrc.com.ar/en/tema1en.php?artist=%a&songname=%s'],
...
$::Options{OPT . 'LyricSite'} = undef
  if $::Options{OPT . 'LyricSite'} && !$sites{$::Options{OPT . 'LyricSite'}};
::SetDefaultOptions(OPT, LyricSite => 'google');

sub addtoolbar {    #my $self=$_[0];
...
sub SongChanged {
    ...
    return
         if defined $self->{ID}
      && !$force
      && ($ID == $self->{ID} || !$self->{follow});
    $self->{ID} = $ID;
    my ($title, $artist) = map ::url_escapeall($_),
      Songs::Get($ID, qw/title artist/);
    return if $title eq '';
    my (undef, $url, $post) = @{$sites{$::Options{OPT . 'LyricSite'}}};
    for ($url, $post) { next unless defined $_; s/%a/$artist/; s/%s/$title/; }
    ::IdleDo('8_mozlyrics' . $self, 1000,
        sub { $self->load_url($url, $post) });
}
...
sub SongChanged {
    ...
    $self->{ID} = $ID;
    my $artist =
      Songs::Get($ID, 'first_artist');    #FIXME add a way to choose artist ?
    return if $artist eq '';
    return
         if defined $self->{Artist}
      && !$force
      && ($artist eq $self->{Artist} || !$self->{follow});
    $self->{Artist} = $artist;
    $artist = ::url_escapeall($artist);
    my $url =
        'http://'
      . $::Options{OPT . 'WikiLocale'}
      . '.wikipedia.org/wiki/'
      . $artist;
...
sub SongChanged {
    ...
    return
      if $self->{url} && !$force && ($url eq $self->{url} || !$self->{follow});
    warn "loading $url\n";
...
sub fill_editbox #if $id => fill entries with existing properties, if $hash => fill entries with content of hash, if neither => empty entries
{
    ...
    $_->signal_connect(changed => \&entry_changed_cb)
      for $entry_title, $entry_url;
...
sub entry_changed_cb {
    my $self    = ::find_ancestor($_[0], __PACKAGE__);
    my $editbox = $self->{editbox};
    $self->{button_save}->set_sensitive($editbox->{entry_title}->get_text ne ''
          && $editbox->{entry_url}->get_text ne '');
}
