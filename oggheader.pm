...
#warn "old size $self->{commentpack_size}, need : ".length($$newcom_packref)."\n";
    if ($self->{commentpack_size} >= length $$newcom_packref) {
        ...
        while ($left) {
            ...
            substr $$pageref, $offset, $size,
              substr($$newcom_packref, $offset2, $size);
            $offset2 += $size;
...
#concatenate newly generated comment packet and setup packet from the original file in $data, and compute the segments in @segments
    ...
    my $continued   = 0;
    {
        ...
        my $page = pack('a4aa x8 a4 V x4 C',
            'OggS', $version, $continued, $serial, $pagenb++, $nbseg)
          . $segments
...
sub _ReadInfo {
    ...
    if (my $packref = _read_packet($self, PACKET_INFO)) {
        my %info;
        @info{
            qw/version channels rate bitrate_upper bitrate_nominal bitrate_lower/
        } = unpack 'x7 VCV V3 C', $$packref;
        return \%info;
    }
    else {
    ...
    if (my $packref = _read_packet($self, PACKET_COMMENT)) {
          ...
          )    #upgrade old embedded pictures format to metadata_block_picture
        {
            ...
            for my $i (0 .. $#$covers) {
                ...
                next unless $data;
                my @val = (
                    map($comments{"coverart$_"}[$i],
                        qw/mime type description/),
                    decode_base64($data)
                );
...
sub _PackComments {
    ...
    my $packet = pack 'Ca6 V/a* V (V/a*)*', PACKET_COMMENT, 'vorbis',
      $self->{vorbis_string}, scalar @comments, @comments;
...
sub _read_page_header {
    ...
    my ($captpat, $ver, $flags, $granule, $sn, $nbseg) =
      unpack 'a4CC a8 a4 x8 C', $buf;
...
#$crc=($crc<<8)^$crc_lookup[ ($crc>>24)^vec($$pageref,$_,8) ] #doesn't work if perl use 64bits
        $crc =
          (($crc << 8) & 0xffffffff)
          ^ $crc_lookup[($crc >> 24) ^ vec($$pageref, $_, 8)]
...
sub _skip_to_last_page {
    ...
    {    #@_=unpack "a4CC a8 VVVC",$1;
        ...
        $granule = $2
          unless $2 eq "\xff\xff\xff\xff\xff\xff\xff\xff"
