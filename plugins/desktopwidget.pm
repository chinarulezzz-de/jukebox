...
sub prefbox {
    ...
    $Treeview->append_column(
        Gtk2::TreeViewColumn->new_with_attributes(
            'active', $togglerenderer, active => 1
        )
    );
    my $renderer = Gtk2::CellRendererText->new;
    $Treeview->append_column(
        Gtk2::TreeViewColumn->new_with_attributes(
            'name', $renderer, text => 2
        )
    );
    ...
    $vbox->pack_start($_, ::FALSE, ::FALSE, 2)
      for ::Hpack($layoutlabel, $LayoutCombo, $add), $sw, $OptionsBox;
    ::weaken($OptionsBox);
    ::weaken($Treeview);
    ::weaken($LayoutCombo);
    Fill();
...
sub Fill {
    ...
    for my $key (sort keys %$DWlist) {
        ...
        $store->set($iter, 0, $key, 1, !$DWlist->{$key}{dw_inactive}, 2,
            $name);
        $Treeview->get_selection->select_iter($iter)
          if $selected && $key eq $selected;
    }
}
...
sub FillOptions {
    ...
    my $textcolor = Gtk2::ColorButton->new_with_color(
        Gtk2::Gdk::Color->parse($opt->{DefaultFontColor}));
...
#my $set_textcolor= ::NewPrefCheckButton(OPT.'set_textcolor',_"Change default text color", cb=>\&init, widget=>$textcolor, horizontal=>1);
    ...
    $spinx->set_digits(0);
    $spiny->set_digits(0);
    $adjx->signal_connect(value_changed =>
          sub { $opt->{x} = $_[0]->get_value; MoveWindow($key); });
    $adjy->signal_connect(value_changed =>
          sub { $opt->{y} = $_[0]->get_value; MoveWindow($key); });

    my $screen   = Gtk2::Gdk::Screen->get_default;
    my $monitors = $screen->get_n_monitors;
    my $adj_mon =
      Gtk2::Adjustment->new($opt->{monitor}, 0, $monitors - 1, 1, 2, 0);
    my $spin_mon = Gtk2::SpinButton->new($adj_mon, 0, 0);
    $spin_mon->set_sensitive(0) if $monitors < 2;
    $adj_mon->signal_connect(value_changed =>
          sub { $opt->{monitor} = $_[0]->get_value; MoveWindow($key); });
    ...
    my $adjw =
      Gtk2::Adjustment->new($opt->{w}, 1,
        ::max(2000, $opt->{w}, Gtk2::Gdk::Screen->get_default->get_width),
        10, 50, 0);
    my $adjh =
      Gtk2::Adjustment->new($opt->{h}, 1,
    ...
    $adjw->signal_connect(value_changed =>
          sub { $opt->{w} = $_[0]->get_value; ResizeWindow($key); });
    $adjh->signal_connect(value_changed =>
