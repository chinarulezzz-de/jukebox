...
sub receiving_cb {
    my $self = $_[2];
    return 1
      if read $self->{content_fh}, $self->{content}, 1024,
      length($self->{content});
    ...
    my $filename;
    while ($self->{ebuffer}
        =~ m#^  Content-Disposition:\s*\w+\s*;\s*filename(\*)?=(.*)$#mgi)
    {
        $filename = $2;
        my $rfc5987 = $1;
    ...
    if ($enc) {
        if ($enc eq 'gzip' && $gzip_ok) {
            ...
            IO::Uncompress::Gunzip::gunzip(\$gzipped, \$self->{content})
              or do {
                warn
                  "simple_http_wget : gunzip failed: $IO::Uncompress::Gunzip::GunzipError\n";
                $result = 'gunzip error';
              };
        }
        else {
...
sub abort {
    ...
    while (waitpid(-1, WNOHANG) > 0) { }    #reap dead children
    $self->{pid} = $self->{content_fh} = $self->{error_fh} = $self->{watch} =
      $self->{ewatch} = undef;
}
