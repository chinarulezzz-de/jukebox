...
my @towrite
  ; # Needed to avoid progress bar overflow in save_fields when called from mass_download
...
# my $hbox_picsize = ::Hpack($spin_picsize, '-', $btn_amg);
    ...
    my $lbl_preview = Label::Preview->new(
        ...
        preview  => sub {
            ...
            my $t =
              ::pathfilefromformat($::SongID, $::Options{OPT . 'PathFile'},
                undef, 1);
    ...
    for my $f (@showfields) {
        push(@chk_show,
            ::NewPrefCheckButton(OPT . 'Show' . $f->{short} => $f->{long}))
          if $f->{active};
    }
    ...
    return ::Vpack($frame_cover, $frame_review, $frame_fields, $frame_layout,
        [$btn_download, $cmb_download]);
}

sub mass_download {
    my $self = bless {
    }; # the context widget was meant to be used as self in methods of this plugin, but when downloading all there is no gtk widget to use. Using a blessed hash instead should work as well as long as no gtk2 function is used on it
...
sub download_one {
    ...
    warn
      "Albuminfo: mass download in progress... $self->{progress}/$self->{end}\n"
      if $::debug;
    ...
    if ($::Options{OPT . 'mass_download'} ne 'all' && $file && -r $file) {
        if ($::Options{OPT . 'SaveFields'}) {
            ::IdleDo('9_load_albuminfo' . $self,
                undef, \&load_file, $self, $ID, $file, 1, \&download_one);
        }
        else {
            ::IdleDo('9_download_one' . $self, undef, \&download_one, $self);
        }
        ...
        $self->{waiting} = Simple_http::get_with_cb(
            ...
            cache => 1,
            cb =>
              sub { $self->load_search_results($ID, 1, \&download_one, @_) }
        );
    }
}
...
sub new {
    ...
    my $update_savebutton_visible =
      sub { $_[0]->set_visible(!$::Options{OPT . 'AutoSave'}); };
    ::Watch($savebutton,
        plugin_albuminfo_option_save => $update_savebutton_visible);
    ...
    $searchbutton->signal_connect(
        clicked => sub {
            my $self = ::find_ancestor($_[0], __PACKAGE__);
            if   ($_[0]->get_active()) { $self->manual_search() }
            else                       { $self->song_changed() }
        }
    );
...
# Year is a 'Glib::String' to avoid printing "0" when year is missing. Caveat: will give wrong sort order for albums released before year 1000 or after year 9999 :)
    ...
    my %coladded;
    for my $col (split(/\s+/, $::Options{OPT . 'Columns'} || ''),
        qw/album artist genre year/)
    {
 ...
 # Recreate the header label to be able to catch mouse clicks in column header:
    ...
    $scrwin->signal_connect(
        key_press_event => sub {
            song_changed($_[0])
              if $_[1]->keyval == $Gtk2::Gdk::Keysyms{Escape};
        }
    );
    $searchview->show_all()
    ...
    $self->{SaveOptions} =
      \&SaveOptions;    #called when widget is removed or when saving options
    return $self;
}
    ...
    if ($event->button == 1) {
        ...
        my $menu = ::BuildChoiceMenu(
            ...
            code => sub {
                ...
                $cols[0]->set_visible(1)
                  unless grep $_->get_visible,
                  @cols;    #make sure one column is visible
            },
        );
        $menu->show_all;
...
sub button_release_cb {
    ...
    for my $tag ($iter->get_tags) {
        if ($tag->{url}) {
            ...
            Songs::Set(Songs::MakeFilterFromGID('album', $aID)->filter(),
                [$field => $tag->{val}]);
        }
    }
    return ::FALSE;
...
sub cover_popup {
    ...
    if (my $img = AAPicture::newimg(album => $aID, $picsize)) {
        ...
        $apic->modify_bg('GTK_STATE_SELECTED',
...
sub print_review {
    ...
    my $tag_b =
      $buffer->create_tag(undef, weight => Gtk2::Pango::PANGO_WEIGHT_BOLD);
    my $tag_i = $buffer->create_tag(undef, style => 'italic');
    my $iter  = $buffer->get_start_iter();

    for my $f (@showfields) {
        if (   $fields->{$f->{short}}
            && $::Options{OPT . 'Show' . $f->{short}}
            && $f->{active})
        {
            ...
            if ($f->{multi}) {    # genres, moods, styles and themes.
                ...
                $buffer->insert_pixbuf($iter,
                    Songs::Stars($fields->{rating}, 'rating'));
            }
            else {
...
sub print_results {
    ...
    for (@$result) {
        $store->set(
            $store->append, 0, $_->{album}, 1, $_->{artist}, 2,
            $_->{genres},   3, $_->{year},  4, $_->{url},    5,
            $_->{order}
        );
    }
}
...
sub album_changed {
    ...
    unless ($force) {    # Try loading from file.
        ...
        if ($file && -r $file) {
            ::IdleDo('9_load_albuminfo' . $self,
                undef, \&load_file, $self, $ID, $file, 0, undef);
            return;
        }
    }
    warn "Albuminfo: fetching search results from $url\n" if $::debug;
...
sub update_titlebox {
    ...
    $self->{Ltitle}->set_markup(
        AA::ReplaceFields($aID, "<big><b>%a</b></big>", "album", 1));
    $self->{Lstats}
      ->set_markup(AA::ReplaceFields($aID, '%b « %y\n%s, %l', "album", 1));
...
sub load_search_results {
    my ($self, $ID, $md, $cb, $html, %prop) = @_
      ; # $md = 1 if mass_download, 0 otherwise. $cb = callback function if mass_download, undef otherwise.
    delete $self->{waiting};
    my $result = parse_amg_search_results($html, $prop{type})
...
# Pick the first entry with the right artist and year, or if not: just the right artist.
        if (::superlc($entry->{artist}) eq ::superlc($artist)) {
            if (!$url || ($entry->{year} && $entry->{year} == $year)) {
                warn
                  "Albuminfo: hit in search results: $entry->{album} by $entry->{artist} from $entry->{year} ($entry->{url})\n"
                  if $::debug;
                $url = $entry->{url};
            }
...
sub parse_amg_search_results {
    ...
    for my $info (split /<div class="info"/, $html) {
        my ($url, $album) = $info
          =~ m#<div class="title".*<a href="(http://www\.allmusic\.com/album/[^"]+)"[^>]*>([^<]+)#i;
        next unless defined $url;
        my %hash = (order => $i++, album => $album, url => $url);
        for my $field (qw/artist year genres/) {
            my ($value) =
              $info =~ m#<div class="$field"[^>]*>\s*(.*?)\s*</div>#i;
...
sub parse_amg_album_page {
    ...
    $result{author} = $1
      if $html
      =~ m|class="review-author headline">[^<]*by <span itemprop="author">(.*?)</span>|i;
    ...
    $result{rls_date} = $1
      if $html
      =~ m|class="release-date">\s*<h\d>Release Date</h\d>\s*<span>([^<]+)</span>|i;
    $result{rec_date} = $1
      if $html
      =~ m|class="recording-date">\s*<h\d>Recording Date</h\d>\s*<span>([^<]+)</span>|i;
    $result{time} = $1
      if $html
      =~ m|class="duration">\s*<h\d>Duration</h\d>\s*<span>([^<]+)</span>|i;
    $result{amgid}  = $1 if $html =~ m|AMG Pop ID.*?R\s*(\d+)\s*</span>|i;
    $result{rating} = 10 * $1
      if $html
      =~ m|<div class="allmusic-rating rating-allmusic-7" itemprop="ratingValue">\s*(\d+)\s*</div>|i;
    my ($genrehtml) =
      $html =~ m|class="genres?">\s*<h\d>Genres?</h\d>\s*<div>(.*?)</div>|i;
    (@{$result{genre}}) = $genrehtml =~ m|<a href="[^"]+">([^<]+)</a>|ig
      if $genrehtml;
    my ($stylehtml) =
      $html =~ m|class="styles">\s*<h\d>Styles</h\d>\s*<div>(.*?)</div>|i;
    (@{$result{style}}) = $stylehtml =~ m|<a href="[^"]+">([^<]+)</a>|ig
      if $stylehtml;
    my ($moodshtml) =
      $html =~ m|class="moods">\s*<h\d>Album Moods</h\d>\s*<div>(.*?)</div>|i;
    (@{$result{mood}}) = $moodshtml =~ m|<a href="[^"]+">([^<]+)</a>|ig
      if $moodshtml;
    my ($themeshtml) =
      $html =~ m|class="themes">\s*<h\d>Themes</h\d>\s*<div>(.*?)</div>|i;
    (@{$result{theme}}) = $themeshtml =~ m|<a href="[^"]+">([^<]+)</a>|ig
      if $themeshtml;
...
sub load_file {
    ...
    if (open(my $fh, '<', $file)) {
        ...
        if ($::Options{OPT . 'StyleAsGenre'} && $self->{fields}->{style}) {
            @{$self->{fields}->{genre}} =
              ::uniq(@{$self->{fields}->{style}}, @{$self->{fields}->{genre}});
        }
        $self->print_review() unless $md;
        if ($::Options{OPT . 'SaveFields'}) {
            push(@towrite, [$ID, %{$self->{fields}}]);
...
sub save_fields {
    ...
    warn "Albuminfo: Saving tracks on $album ("
      ...
      . " in queue).\n"
      if $::debug;
    my $IDs = Songs::MakeFilterFromGID('album', Songs::Get_gid($ID, 'album'))
      ->filter();    # Songs on this album
    my @updated_fields;
    ...
    if (@updated_fields) {
        $save_fields_lock = 1;
        Songs::Set($IDs, \@updated_fields,
            callback_finish => sub { $save_fields_lock = 0; save_fields(); });
    }
    else {
