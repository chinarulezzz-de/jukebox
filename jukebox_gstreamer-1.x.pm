...
BEGIN {
    ...
    if ($reg->lookup_feature('equalizer-10bands')) { $GST_EQ_ok = 1; }
        ...
        warn
          "gstreamer1 plugin 'equalizer-10bands' not found -> equalizer not available\n";
    }
    if ($reg->lookup_feature('rglimiter') && $reg->lookup_feature('rgvolume'))
    {
        ...
        warn
          "gstreamer1 plugins 'rglimiter' and/or 'rgvolume' not found -> replaygain not available\n";
    }
    if ($reg->lookup_feature('rganalysis')) { $GST_RGA_ok = 1; }
    else {
        warn
          "gstreamer1 plugins 'rganalysis' not found -> replaygain analysis not available\n";
    }

#some functions that should be accessible from perl but are not currently (Glib::Object::Introspection-0.027)
    *GStreamer1::Bin::add_many = sub { my $b = shift; $b->add($_) for @_ }
      unless *GStreamer1::Bin::add_many{CODE};
    *GStreamer1::Element::link_many = sub {
        while (@_ > 1) { my $e = shift; $e->link($_[0]) }
      }
      unless *GStreamer1::Element::link_many{CODE};
...
# setup the gstvideooverlay interface (used for telling playbin the window ID to use for visuals)
    ...
    if (1) {
        ...
        if (!$@) {
            ...
            $support_install_missing = 1
              if GStreamer1::Pbutils::install_plugins_supported();
            warn
              "gstreamer says Installing missing plugins not supported by this system\n"
              if $::debug;
        }
        else {
            warn
...
sub init {
    ...
    $::Options{gst_sink} ||= (
        grep ($reg->lookup_feature($_ . 'sink'),
            qw/autoaudio pulse alsa oss oss4/),
        'autoaudio'
...
sub create_playbin {
    ...
    $pb->signal_connect(
        "notify::volume" => sub {
            Glib::Idle->add(\&VolumeChanged, $_[0])
              unless $VolumeHasChanged++;
        },
        100000
      )
...
#	$bus->signal_connect('message' => \&bus_message);
    $bus->signal_connect('message::element' => \&bus_message_missing_plugin)
      if $support_install_missing;
    ...
    $pb->signal_connect(about_to_finish => \&about_to_finish)
...
sub bus_message_missing_plugin {
    ...
    my $details =
      GStreamer1::Pbutils::missing_plugin_message_get_installer_detail($msg);
...
sub create_sink {
    ...
    my $sink =
      GStreamer1::ElementFactory::make($sinkname . 'sink' => $sinkname);
...
sub init_sink {
    ...
    if ($sink->{EQ}) {
        my $preamp =
          GStreamer1::ElementFactory::make('volume' => 'equalizer-preamp');
        my $equalizer =
    ...
    if (my $custom = $::Options{gst_custom}) {
        ...
        if ($elem && $elem->isa('GStreamer1::Bin')) {
            ...
            $first = ($first->list_iterate_sorted)[0]
              while $first->isa('GStreamer1::Bin');
            $last = ($last->list_iterate_sorted)[-1]
              while $last->isa('GStreamer1::Bin');
            $elem->add_pad(
                GStreamer1::GhostPad->new(
                    'sink', $last->get_static_pad('sink')
                )
            );
            $elem->add_pad(
                GStreamer1::GhostPad->new(
                    'src', $first->get_static_pad('src')
                )
            );
        }
...
sub Play {
    ...
    if (   $self->{already_next_song}
        && $self->{already_next_song} eq $file
        && $keep
        && !$skip)
    {
...
sub SetVolume {
    ...
    $self->{volume_busy} = 1;
    my $pb = $self->{playbin};
    $pb->set(volume => (($::Mute || $::Volume) / 100)**3, mute => !!$::Mute)
      if $pb;    #use a cubic volume scale
    $self->{volume_busy}    = 0;
...
sub set_equalizer_preamp {
    my ($self, $volume) = @_;
    my $preamp =
      $self->{playbin} && $self->{playbin}->get_by_name('equalizer-preamp');
...
sub set_equalizer {
    my ($self, $values) = @_;
    my $equalizer =
      $self->{playbin} && $self->{playbin}->get_by_name('equalizer');
...
sub EQ_Import_Presets {
    ...
    for my $name (@{$equalizer->get_preset_names}) {
        next
          if $::Options{equalizer_presets}{$name}
          ;    #ignore if one by that name already exist
        $new++;
        $equalizer->load_preset($name);
        $::Options{equalizer_presets}{$name} = join ':',
          map $equalizer->get('band' . $_), 0 .. 9;
    }
...
sub remove_visuals {
    ...
    ::weaken($_)
      for
      @$wlist;    #re-weaken references (the grep above made them strong again)
...
sub list_visuals {
    my @visuals;
    my $reg = GStreamer1::Registry::get;
    for my $plugin (@{$reg->get_plugin_list}) {    #warn $plugin->get_name;
...
sub Analyse {
    ...
    unless ($RGA_pipeline) {
        ...
        $bus->signal_connect(
            'message::error' => sub {
                warn "ReplayGain analysis error : "
...
sub newpad_cb {
    my ($decodebin, $pad) = @_;
    my $audiopad =
      $RGA_pipeline->get_by_name('RGA_audiobin')->get_static_pad('sink');
...
sub bus_message_tag {
    ...
    my (%tags, $count);
    for my $field (
        qw/replaygain-reference-level replaygain-track-gain replaygain-track-peak replaygain-album-gain replaygain-album-peak/
      )
    {
...
sub init {
    ...
    return unless $ok;
    return
      bless {EQ => $GST_EQ_ok, visuals => $GST_visuals_ok, RG => $GST_RG_ok},
      __PACKAGE__;
}
...
sub create_sink {
    ...
    my ($aconv, @elems) = map GStreamer1::ElementFactory::make($_ => $_),
      @pipeline;
    ...
    $stream->{sockets} = {};
    $sink->add_pad(
        GStreamer1::GhostPad->new('sink', $aconv->get_static_pad('sink')));
