...
our @MenuQueue = (
    ...
    {   include => sub {
            my $menu = $_[1];
            my @modes =
              map { $_ => $::QActions{$_}{long} } ::List_QueueActions(0);
            ::BuildChoiceMenu(
                \@modes,
                menu         => $menu,
                ordered_hash => 1,
    ...
    {   include => sub {
            my $menu  = $_[1];
            my @modes = map { $_ => $::QActions{$_}{long_next} } grep $_ ne '',
              ::List_QueueActions(1);
            ::BuildChoiceMenu(
                \@modes,
                menu            => $menu,
...
our %Widgets = (
    ...
    Queue => {
        ...
        tip => sub {
            if ($::NextAction) { return $::QActions{$::NextAction}{long_next} }
    ...
    Pos => {
        ...
        click1 => sub {
            ::ChooseSongs([::GetNeighbourSongs(5)])
              unless $::RandomMode || @$::Queue;
        },
        update => sub {
            my $t =
              (@$::ListPlay == 0) ? ''
              : @$::Queue ? ::__n("%d song in queue", "%d songs in queue",
                scalar @$::Queue)
              : !defined $::Position
    ...
    Title => {
        ...
        click3 => sub {
            my $ID = ::GetSelID($_[0]);
            ::PopupContextMenu(\@::SongCMenu,
                {mode => 'P', self => $_[0], IDs => [$ID]})
              if defined $ID;
        },
    ...
    Album => {
        ...
        click1  => sub {
            my $ID = ::GetSelID($_[0]);
            ::PopupAA('album', from => Songs::Get_gid($ID, 'artists'))
              if defined $ID;
        },
        click3 => sub {
            my $ID = ::GetSelID($_[0]);
...
#		font	=> 'Monospace',
        event => 'Time',
        click1 =>
          sub { $_[0]{remaining} = !$_[0]{remaining}; $_[0]->update_time; },
    ...
    TimeBar => {
        ...
        fields  => 'length',
        schange => sub {
            $_[0]->set_max(defined $_[1] ? Songs::Get($_[1], 'length') : 0);
        },
        set    => sub { ::SkipTo($_[1]) },
    ...
    Cover => {
        ...
        oldopt1 => 'maxsize',
        schange => sub {
            my $key = (defined $_[1]) ? Songs::Get_gid($_[1], 'album') : undef;
            $_[0]->set($key);
        },
        click1 => \&PopupSongsFromAlbum,
        click3 => sub {
    ...
    ArtistPic => {
        ...
        schange => sub {
            my $key =
              (defined $_[1]) ? Songs::Get_gid($_[1], 'artists') : undef;
            $_[0]->set($key);
        },
    ...
    TabbedLists => {
        ...
        options_for_list =>
          'songlist songtree sort songxpad songypad no_typeahead cols grouping',
    },
    Context => {
        class       => 'Layout::NoteBook',
        EndInit     => \&Layout::NoteBook::EndInit,
    ...
    PlayItem => {
        ...
        updatemenu => sub {
            ...
            ::BuildMenu(\@Browser::MenuPlaying,
                {self => $_[0], songlist => $sl},
                $_[0]->get_submenu);
        },
    },
    LSortItem => {
    ...
    LayoutItem => {
        ...
        updatemenu => sub {
            ::BuildChoiceMenu(
                ...
                code  => sub {
                    $::Options{Layout} = $_[1];
                    ::IdleDo('2_ChangeLayout', 500, \&::CreateMainWindow);
                },
    ...
    SeparatorMenuItem => {New => sub { Gtk2::SeparatorMenuItem->new },},
    Refresh           => {
        class    => 'Layout::Button',
        size     => 'menu',
        stock    => 'gtk-refresh',
    ...
    HSeparator => {New => sub { Gtk2::HSeparator->new },},
    VSeparator => {New => sub { Gtk2::VSeparator->new },},
    Choose     => {
        class    => 'Layout::Button',
        stock    => 'gtk-add',
    ...
    ChooseRandAlbum => {
        ...
        activate => sub {
            ...
            if (my $ac = $_[0]{action}) { ::DoActionForList($ac, $list); }
            else {
    ...
    EditListButtons => {class => 'EditListButtons',},
    QueueActions    => {class => 'QueueActions',},
    Fullscreen      => {
        class          => 'Layout::Button',
...
sub get_layout_list {
    ...
    @list = grep defined $Layouts{$_}{Type} && $Layouts{$_}{Type} =~ m/$type/,
      @list
      if $type;
...
sub ParseLayout {
    ...
    if ($first =~ m/^\[([^]=]+)\](?:\s*based on (.+))?$/) {
        if (defined $2 && !exists $Layouts{$2}) {
            warn
              "Ignoring layout '$1' because it is based on unknown layout '$2'\n";
            return;
        }
        $name = $1;
        if (defined $2) {
...
sub SaveWidgetOptions #Save options for this layout by collecting options of its widgets
{
    ...
    for my $widget (@widgets) {
        ...
        if (my $keys = $widget->{options_to_save}) {
            $opt->{$_} = $widget->{$_}
              for grep defined $widget->{$_}, split / /, $keys;
        }
        next unless $opt;
...
sub InitLayout {
    ...
    $self->{KeyBindings} = ::make_keybindingshash($boxes->{KeyBindings})
      if $boxes->{KeyBindings};
    $self->{widgets} = {};
...
sub CreateWidgets {
    ...
    for my $key (keys %$boxes) {
        ...
        my $group = $opt1->{group};
        $opt1->{group} = $defaultgroup . (length $group ? "-$group" : '')
          unless $group =~ m/^[A-Z]/;
        my $box = $widgets->{$key} = $type->{New}($opt1);
        $box->{$_} = $opt1->{$_}
          for grep exists $opt1->{$_},
          qw/group tabicon tabtitle maxwidth maxheight expand_weight/;
        ApplyCommonOptions($box, $opt1);

        $box->{name} = $fullname;
        $box->set_border_width($opt1->{border})
          if $opt1 && exists $opt1->{border} && $box->isa('Gtk2::Container');
        $box->set_name($key);
    ...
    while (@boxlist) {
        ...
        for my $name (@names) {
            ...
            if ($widget) {
                if ($widget->parent) {
                    warn
                      "layout error: $name already has a parent -> can't put it in $key\n";
                    next;
                }
                ...
                $placeholder =
                  Layout::PlaceHolder->new($type, $box, $placeholder,
                    $packoptions);
    ...
    for my $key (grep m/^[HV]Size/, keys %$boxes) {
        ...
        if ($names[0] =~ m/^\d+$/) {
            ...
            $_->set_size_request(@req)
              for grep defined, map $widgets->{$_}, @names;
            next if @names == 1;
        }
    ...
    $self->{layoutdepth}--;
    my @noparentboxes =
      grep m/^(?:[HV][BP]|[AMETNFSW]B|FR)/ && !$widgets->{$_}->parent,
      keys %$boxes;
    if (@noparentboxes == 0) {
        ...
        warn
          "layout error: (@noparentboxes) have no parent -> can't find toplevel box\n";
    }
...
sub NewWidget {
    ...
    $opt2 ||= {};
    my %options =
      (group => '', %$ref, %$opt1, %$opt2, name => $namefull, %$global_opt);
    $options{font} ||= $global_opt->{DefaultFont}
      if $global_opt->{DefaultFont};
    my $group = $options{group}; #FIXME make undef group means parent's group ?
    my $defaultgroup = $options{default_group} || 'default_group';
    $options{group} = $defaultgroup . ($group =~ m/^\w/ ? '-' : '') . $group
      unless $group
      =~ m/^[A-Z]/;    #group local to window unless it begins with uppercase
    my $widget =
        $ref->{class}
    ...
    $widget->{$_} = $options{$_}
      for 'group', split / /, ($ref->{options} || '');
    $widget->{$_} = $options{$_}
      for grep exists $options{$_},
      qw/tabtitle tabicon tabrename maxwidth maxheight expand_weight/;
    ...
    $widget->signal_connect(button_press_event => \&Button_press_cb)
      if $widget->{actions};
    ...
    if (my $cursor = $options{cursor}) {
        $widget->signal_connect(
            realize => sub {
                ...
                if ($widget->isa('Gtk2::EventBox')
                    && !$widget->get_visible_window)
...
# the only way to get to the input-only gdkwin is looking at all the children of its parent :(
                    for my $child ($gdkwin->get_children) {
                        my $w = Glib::Object->new_from_pointer(
                            $child->get_user_data);
...
sub ApplyCommonOptions # apply some options common to both boxes and other widgets
{
    ...
    if ($opt->{hover_layout}
      ) # only works with widgets/boxes that have their own gdkwindow (put it into a WB box otherwise)
    {
        $widget->{$_} = $opt->{$_}
          for qw/hover_layout hover_delay hover_layout_pos/;
...
sub ShowHide {
    ...
    $show = !grep $_ && $_->visible, map $self->{widgets}{$_}, split /\|/,
      $names
      unless defined $show;
...
sub KeyPressed {
    ...
    return 0
      if !$after
      && $focused
      && ($focused->isa('Gtk2::Entry') || $focused->isa('Gtk2::SpinButton'));
    my $mod;
...
sub SetFocusOn {
    my ($self, $name) = @_;
    while ($name =~ s#^([^/]+)/##
      ) # if name contains slashes, divide it into parent and child, where parent can be an Embedded layout or a TabbedLists/Context/NB
    {
        $self = $self->{widgets}{$1};
        return unless $self;
    }
    ...
    $combo->signal_connect(
        changed => sub {
            ...
            if ($sort =~ m/^EDIT (.)$/) {
                ...
                if ($1 eq 'O') {
                    ::EditSortOrder(undef, $::Options{Sort}, undef,
                        \&::Select_sort);
                }
                elsif ($1 eq 'R') {
                    ::EditWeightedRandom(undef, $::Options{Sort}, undef,
                        \&::Select_sort);
                }
            }
...
sub PlayOrderComboUpdate {
    ...
    for my $name (sort keys %{$::Options{SavedSorts}}) {
        ...
        $store->set($iter = $store->append,
            0, $name, 1, $sort, 2, 'gtk-sort-ascending');
        $found = $iter if $sort eq $check;
    }
    if (!$found) {
        $store->set($iter = $store->append,
...
sub FilterMenu {
    ...
    $item->signal_connect(
        activate => sub {
            ::EditFilter(undef, $::SelectedFilter, undef,
                sub { ::Select(filter => $_[0]) });
        }
    );
    $menu->append($item);
    if (my @SavedLists = ::GetListOfSavedLists()) {
        ...
        for my $list (@SavedLists) {
            ...
            $item->set_draw_as_radio(1);
            $item->set_active(1)
              if defined $::ListMode && $list eq $::ListMode;
...
sub VisualsMenu {
    ...
    my $cb =
      sub { $::Play_package->set_visual($_[1]) if $::Play_package->{visuals}; };
...
sub UpdateLabelsIcon {
    ...
    for my $stock (Songs::Get_icon_list($table->{field}, $::SongID)) {
        ...
        $count++;
        $table->attach($img, $col, $col + 1, $row, $row + 1, 'shrink',
            'shrink', 1, 1);
        if (++$row >= 1) { $row = 0; $col++ }
    }
    $table->show_all;
...
sub new {
    ...
    if (my $opt = $layout =~ m/^[^(]+\(.*=/) {
        ($layout, $opt0) =
          $layout =~ m/^([^(]+)\((.*)\)$/;    #separate layout id and options
    ...
    %options =
      (borderwidth => 0, %$opt1, %{$opt2->{Window} || {}}, %options, %$opt0);
...
#warn "window options (layout=$layout) :\n";warn " $_ => $options{$_}\n" for sort keys %options;
    ...
    if (my $mode = $options{ifexist}) {
        my ($window) =
          grep $_->isa('Layout::Window') && $_->{uniqueid} eq $uniqueid,
        ...
        if ($window) {
            ...
            } # destroying previous window make it save its settings, then restart new() from the start with new $opt2 but the same original arguments, add ifexists=>0 to make sure it doesn't loop
    ...
    $self->signal_connect(
        focus_in_event => sub { $_[0]{last_focused} = time; 0; });
...
#		);
    ...
    $self->set_opacity($self->{opacity})
      if exists $self->{opacity} && $self->{opacity} != 1;
    ::QHasChanged('Windows');
    return $self;
}
...
sub init {
    ...
    $self->set_workspace(delete $::CmdLine{workspace})
      if exists $::CmdLine{workspace};
...
sub SetWindowOptions {
    ...
    $self->{hidden} = {$opt->{hidden} =~ m/(\w+)(?::?(\d+x\d+))?/g}
      if $opt->{hidden};

    $self->{size} = $self->{fixedsize} = $opt->{fixedsize}
      if $opt->{fixedsize};
...
sub Resize {
    ...
    my (undef, undef, $monitorwidth, $monitorheight) =
    ...
    if ($self->{options}{DEFAULT_OPTIONS}) {
        $monitorwidth  -= 40;
        $monitorheight -= 80;
    } # if using default layout size, reserve some space for potential panels and decorations #FIXME use gdk_screen_get_monitor_workarea once ported to gtk3
...
sub Position {
    ...
    my ($monitor, $x, $xalign, $y, $yalign) = $pos
      =~ m/(?:(\d+)@)?\s*([+-]?\d+%?)(?:([+-]\d+)%)?\s*x\s*([+-]?\d+%?)(?:([+-]\d+)%)?/;
...
sub make_transparent {
    ...
    while (my $widget = shift @children) {
        push @children, $widget->get_children
...
sub set_background_skin {
    ...
    $self->{pixbuf} =
      Skin::_load_skinfile($file, $crop, $self->{global_options});
    ...
    $self->signal_connect(
        style_set =>
          sub { warn "style set : @_" if $::debug; $_[0]->set_style($_[2]); },
        $self->get_style
...
sub resize_skin_cb #FIXME needs to add a delay to better deal with a burst of resize events
{
    ...
    { #my ($pixmap,$mask)=$pb->render_pixmap_and_mask(1); #leaks X memory for Gtk2 <1.146 or <1.153
        ...
        $pb->render_to_drawable($pixmap, Gtk2::Gdk::GC->new($self->window),
...
sub init {
    ...
    $self->signal_connect(leave_notify_event =>
          sub { $_[0]->CheckCursor if $_[1]->detail ne 'inferior'; 0; });
    $self->SUPER::init;
}
...
sub CheckCursor # StartDestroy if popup is not ancestor of widget under cursor and cursor isn't grabbed (menu)
{
    ...
    return 1
      if $self->get_display
      ->pointer_is_grabbed;    # to prevent destroying while a menu is open

    if (my $sicon = $self->{popped_from}) {
        return 1
          if $sicon->isa('Gtk2::StatusIcon')
          && OnStatusIcon($sicon);    #check if pointer above statusicon
    }

    my ($gdkwin) = Gtk2::Gdk::Window->at_pointer;
    my $widget =
      $gdkwin ? Glib::Object->new_from_pointer($gdkwin->get_user_data) : undef;
    while ($widget) {
        last if $widget->isa('Gtk2::StatusIcon');
        $widget = ::find_ancestor($widget, 'Layout::Window::Popup');
        last unless $widget;
        return 1
          if $widget == $self;  # don't destroy if cursor is over child of self
...
sub Popup {
    ...
    $addtimeout = 0
      if $self
      && !$self->{destroy_timeout}
      ;    #don't add timeout if there wasn't already one
    $self ||= Layout::Window::Popup->new($widget->{hover_layout}, $widget);
    return 0 unless $self;
    $self->CancelDestroy;
    $self->{destroy_timeout} =
      Glib::Timeout->add($addtimeout, \&DestroyNow, $self)
      if $addtimeout;
    $self->{check_timeout} ||= Glib::Timeout->add(400, \&CheckCursor, $self)
      if $widget->isa('Gtk2::StatusIcon') && !$addtimeout;
    0;
}
...
sub set_hover {
    ...
    if ($widget->isa('Gtk2::StatusIcon')) {
        ...
        $widget->signal_connect(query_tooltip =>
              sub { return if $_[0]{hover_timeout}; &PreparePopup });
    }
    else {
...
sub PreparePopup {
    ...
    if (!$widget->{PoppedUpWindow}) {
        ...
        $widget->{hover_timeout} =
          Glib::Timeout->add($delay, \&HoverPopup, $widget);
    }
    else { Popup($widget) }
    0;
}
...
sub CancelPopup {
    ...
    if (my $self = $widget->{PoppedUpWindow}) {
        $self->StartDestroy;
        $self->{check_timeout}
          ||= Glib::Timeout->add(1000, \&CheckCursor, $self);
    }
}
...
sub new {
    ...
    %children_opt = (%children_opt, %{$opt->{children_opt}})
      if $opt->{children_opt};
...
our %Boxes = (
    ...
    AB => {
        New => sub {
            my %opt =
              (xalign => .5, yalign => .5, xscale => 1, yscale => 1, %{$_[0]});
...
sub PanedPack {
    ...
    if    (!$paned->child1) { $paned->pack1($wg, $expand, $shrink); }
        ...
        warn
          "layout error : trying to pack more than 2 widgets in a paned container\n";
    }
}
...
sub PanedNew {
    ...
    ($self->{size1}, $self->{size2}) = map $_ + 0, split /-|_/, $opt->{size}
      if defined $opt->{size}
    ...
    if (defined $self->{size1}) {
        ...
        $self->set('position-set', 1)
          ; # in case $self->{size1}==0 'position-set' is not set to true if child1's size is 0 (which is the case here as child1 doesn't exist yet)
    }
...
use Glib::Object::Subclass
  Gtk2::Fixed::,
  signals => {size_allocate => \&size_allocate,};
...
sub size_allocate {
    ...
    for my $child ($self->get_children) {
        ...
        $child->size_allocate(
            Gtk2::Gdk::Rectangle->new($ox + $x, $oy + $y, $ww, $wh));
    }
}
...
our @contextmenu = (
    ...
    {   label => "Open existing list",
        ...
        submenu => sub {
            my %h;
            $h{$_->{array}->GetName} = 1
              for grep $_->{name} =~ m/^EditList\d*$/,
              $_[0]{self}->get_children;
...
sub new {
    ...
    $self->{$_} = $opt->{$_}
      for
      qw/group default_child match pages page typesubmenu closebuttons tablist/;
    ...
    $opt->{newbutton} = 0
      unless *Gtk2::Notebook::set_action_widget{CODE}
      ; # Gtk2::Notebook::set_action_widget requires gtk+ >= 2.20 and perl-Gtk2 >= 1.23
    if (   $opt->{typesubmenu}
        && $opt->{newbutton}
        && $opt->{newbutton} ne
        'none')    # add a button next to the tabs to show new-tab menu
    {
...
sub EndInit {
    ...
    if (my $pages = delete $self->{pages}) {
        ...
        $pagewidget{"+$_"} = $self->newtab($_)
          for map m/^\+(.+)$/,
          @pagelist;    #recreate dynamic pages (page name begin with +)
        my $i = 0;
        $self->reorder_child($_, $i++)
          for grep $_, map $pagewidget{$_},
          @pagelist;    #reorder pages to the saved order
    }
...
sub newtab {
    ...
    my $wclass =
      $wtype->{issonglist} ? 'list' : $name =~ m/^@/ ? 'layout' : 'context';
    my $group = $self->{group};
    $group = 'Global(' . ::refaddr($self) . '-' . $self->{groupcount}++ . ')'
      if $wclass eq 'list';    # give songlist/songtree their own group

    if ($opt2)                 #new widget => use a new name not already used
    {
        my $n = 0;
        $n++
          while $self->{widgets}{$name . $n}
          || $self->{widgets_opt}{$name . $n};
        $name .= $n;
    }
    ...
    my $widget =
      Layout::NewWidget($name, $opt1, $opt2, {default_group => $group});
    return unless $widget;
    ...
    $self->set_current_page($self->get_n_pages - 1)
      if $setpage;    #set current page to the new page
    return $widget;
}
...
sub insert_default_page {
    ...
    ::IdleDo('5_create_chooser_page', 500, \&create_chooser_page, $self)
      if !$self->get_children && $self->{match};
}
...
sub close_tab {
    ...
    if (   $manual
        && $self->{match}
        && $Layout::Widgets{$name}
        && $Layout::Widgets{$name}{autoadd_type})
    {
    ...
    delete $self->{DefaultFocus}
      if $self->{DefaultFocus} && $self->{DefaultFocus} == $page;
    $self->insert_default_page unless $self->get_children;
}
...
sub SavedLists_changed #remove EditList tab if corresponding list has been deleted
{
    ...
    my @remove =
      grep $_->{name} =~ m/^EditList\d*$/ && !defined $_->{array}->GetName,
      $self->get_children;
...
sub newbutton_cb {
    ...
    ::PopupContextMenu(\@contextmenu,
        {self => $self, type => $self->{typesubmenu}, usemenupos => 1});
    1;
}
...
sub button_press_event_cb {
    ...
    return 0
      unless ::IsEventInNotebookTabs($self, $event)
...
sub pagerename_cb {
    ...
    $entry->set_inner_border(undef)
      if *Gtk2::Entry::set_inner_border{CODE};    #Gtk2->CHECK_VERSION(2,10,0);
    ...
    $entry->show_all;
    $entry->signal_connect(
        key_press_event => sub                    #abort if escape
        {
            my ($entry, $event) = @_;
    ...
    $entry->signal_connect(
        populate_popup => sub { ::weaken($_[0]{popupmenu} = $_[1]); });
    $entry->signal_connect(
        focus_out_event => sub {
            my $entry     = $_[0];
            my $popupmenu = delete $entry->{popupmenu};
            return 0
              if $entry->get_display->pointer_is_grabbed
              && $popupmenu
              && $popupmenu
              ->mapped;  # prevent error when context menu of the entry pops-up
...
sub create_chooser_page {
    ...
    $self->remove($_)
      for $self->get_children;    #remove a previous version of this page
...
sub make_widget_list {
    ...
    for my $name (@names) {
        next
          if grep $name eq $_->{name},
          $self->get_children;    #or use $self->{widgets}{$name} ?
...
sub Widgets_changed_cb #new or removed widgets => check if a widget should be added or removed
{
    ...
    if ($changetype eq 'remove') {
        for my $name (@widgets) {
            $self->close_tab($_)
              for grep $name eq $_->{name}, $self->get_children;
        }
        return;
    }
    my $match = $self->{match};
    ...
    ::IdleDo('5_create_chooser_page', 500, \&create_chooser_page, $self)
      if !$self->get_children || $self->{chooser_mode};
}
...
sub SwitchedPage {
    ...
    if (my $songlist = $SongList::Common::Register{$group}) {
        ...
        ::Watch(
            $self,
            'Selection_' . $group,
            sub { ::HasChanged('Selection_' . $_[0]->{group}); }
        );
...
sub new {
    ...
    unless ($type) {
        return Layout::PlaceHolder::Single->new($boxfunc, $box, $ph,
            $packoptions);
    }
    bless $ph, $class;
...
sub Widgets_changed_cb {
    ...
    if ($changetype eq 'new' && !$ph->{widget}) {
        my $widget = Layout::NewWidget($ph->{name}, $ph->{opt1}, $ph->{opt2},
            {default_group => $ph->{group}});
        return unless $widget;
        $ph->{widget}      = $widget;
        $ph->{SaveOptions} = \&SaveOptions;
...
sub new {
    ...
    if (!ref $stock && $ref->{'state'}) {
        ...
        %hash = %$default
          if ref $default eq
          'HASH';    #make a copy of the default setting if it is a hash
    ...
    if ($opt->{skin}) {
        ...
        if ($opt->{with_text}) {
            ...
            $self->{markup} = $opt->{markup}
              || ($opt->{size} eq 'menu' ? "<small>%s</small>" : "%s");
...
sub UpdateStock {
    ...
    if ($stock =~ m/ /) {
        ...
        $stock = ''
          if $stock eq '.';    #needed ? the result is the same : no icon
...
sub new {
    ...
    my $font =
      $opt->{font} && Gtk2::Pango::FontDescription->from_string($opt->{font});
...
#$self->signal_connect(leave_notify_event => sub {my $m=$_[0]->child->get_label; $m=~s#^<u>##;$m=~s#</u>$##; $_[0]->set_markup($m)});
    ...
    if ($minsize && $minsize =~ m/^\d+p?$/) {
        ...
        else                             # scroll when mouse is over it
        {
            ...
            $self->signal_connect(
                enter_notify_event => \&enter_leave_cb,
                INCR()
            );
            $self->signal_connect(
                leave_notify_event => \&enter_leave_cb,
                -INCR()
            );
        }
    }
    ...
    if (exists $opt->{markup}) {
        ...
        if (my @fields = ::UsedFields($m)) {
            $self->{EndInit} =
              \&init;    # needs $self->{group} set before this can be done
        }
...
sub checksize #extend the requested size so that the string fit in initsize mode (in case the initsize string is not wide enough)
{
    ...
    {         # make it expand up to min(maxwidth,string_width)
        my $label = $self->child;
        $label->get_layout->set_width($emax * Gtk2::Pango->scale)
          if $label->get_ellipsize ne 'none';
...
sub restart_scrollcheck    #only used for autoscroll
{
    ...
    $self->{scrolltimeout}
      ||= Glib::Timeout->add($self->{interval}, \&Scroll, $self);
}
...
sub expose_cb    #only for scrollable labels
{
    ...
    $label->get_style->paint_layout(
        $label->window,          $label->state,
        ::FALSE,                 $event->area,
        $label,                  'label',
        $xoffset - $label->{dx}, $yoffset,
        $layout
    );
    1;
}
...
sub Scroll {
    ...
    my $reached_max =
         ($max < 0)
      || ($dx == 0 && $self->{scroll_inc} < 0)
      || ($dx == $max && $self->{scroll_inc} > 0);

    if ($self->{autoscroll}) {
        $self->{scroll_inc} = -$self->{scroll_inc}
          if $reached_max;    # reverse scrolling
        $self->{scrolltimeout} = $self->{scroll_inc} = 0
          if $max < 0;        # no need for scrolling => stop checks
    }
    else {
...
sub new {
    ...
    if ($opt->{text}) {
        ...
        my $font = $opt->{font};
        $self->modify_font(Gtk2::Pango::FontDescription->from_string($font))
          if $font;
    }
    my $orientation =
    ...
    if ($opt->{skin} && $opt->{handle_skin}) {
        $self =
          Layout::Bar::skin->new($opt) || $self;    # warning : replace $self
    }
    $self->add_events(
        [   qw/pointer-motion-mask button-press-mask button-release-mask scroll-mask/
        ]
    );
...
sub update {
    ...
    if (my $text = $self->{text}) {
        $text = $self->{text_empty}
          if !defined $::SongID && defined $self->{text_empty};
        my $now = $self->{now} || 0;
        my $max = $self->{max} || 0;
...
sub button_press_cb {
    my ($self, $event) = @_;
    $self->{pressed}
      ||= $self->signal_connect(motion_notify_event => \&button_press_cb);
    my ($x, $w) =
      $self->{vertical}
...
sub update_preview_Time {
    ...
    my @labels = grep $_->isa('Layout::Label::Time'),
      values %$h;    #get list of Layout::Label::Time widgets in the layouts
...
sub new {
    my ($class, $opt) = @_;
    my $self = bless Gtk2::EventBox->new, $class;
    my $hskin = $self->{handle_skin} =
      Skin->new($opt->{handle_skin}, undef, $opt);
...
sub expose_cb {
    ...
    if ($self->{vertical}) {
        ...
        $y -= $minh / 2;
        Skin::draw($self, $event, $self->{handle_skin}, $self->{left},
            int($y), $w - $self->{left} - $self->{right}, $minh);
    }
    else {
        ...
        $x -= $minw / 2;
        Skin::draw($self, $event, $self->{handle_skin}, int($x), $self->{top},
            $minw, $h - $self->{top} - $self->{bottom});
    }
    1;
}
...
sub button_press_cb {
    ...
    {
        $self->{pressed} = $self->signal_connect(
            motion_notify_event => \&update_value_direct_mode);
        $self->update_value_direct_mode($event);
        return 1;    # return 1 so that Gtk2::Scale won't get the mouse click
    }
    $self->{pressed} =
      $self->signal_connect(value_changed => \&value_changed_cb);
    return 0;
}
...
sub new {
    ...
    if ((my $refl = $opt->{reflection}) && $::CairoOK) {
        ...
        $self->{reflection} =
          $refl == 1 ? $opt->{r_height} / 100 : $refl / 100;
    ...
    if (my $o = $opt->{overlay}) {
        {
            my ($x, $y, $xy_or_wh, $w, $h, $file) =
              $o =~ m/^(\d+)x(\d+)([-:])(\d+)x(\d+):(.+)/;
            unless (defined $file) {
                warn
                  "Invalid picture-overlay string : '$o' (format: XxY:WIDTHxHEIGHT:FILE)\n";
                last;
            }
            ...
            my $h0 = $pb->get_height;
            warn
              "Bad picture-overlay values : rectangle bigger than the overlay picture\n"
              if $w0 < $w + $x || $h0 < $h + $y;
            my $ws = $w0 / $w;
            my $hs = $h0 / $h;
            $ratio            *= $ws / $hs;
    ...
    $self->signal_connect(expose_event  => \&expose_cb);
    $self->signal_connect(
        destroy => sub { delete $::ToDo{'8_LoadImg' . $_[0]} });
...
sub set {
    ...
    return
      if $self->{key} && join("\x1D", @$key) eq join("\x1D", @{$self->{key}});
    $self->{key} = $key;
    my $col = $self->{aa};
    my @files;
    for my $k (@$key) {
    ...
    $self->signal_connect(
        'map' =>
          sub #undo the temporary settings set in size_allocate_cb for the natural_size mode #FIXME should be simpler
        {
            my $self = $_[0];
            delete $self->{size}
...
sub size_allocate_cb {
    ...
    if (delete $self->{natural_size}
      )  #set temporary settings for natural_size mode #FIXME should be simpler
    {
        $self->set_size_request($w, $h)
          if !defined $self->{size} || $size ne $self->{size};
        $self->{size} = $size;
        return;
    }
...
sub expose_cb {
    ...
    if (!$self->{reflection}) {
        ...
        $self->window->draw_pixbuf($gc, $pixbuf, 0, 0, $x, $y, -1, -1, 'none',
            0, 0);
    }
    else {
...
sub draw_with_reflection {
    ...
    my $pattern =
      Cairo::LinearGradient->create(0, $h, 0, $h - $rh * (1 / $scale));
...
sub collage {
    ...
    my $pixbuf =
      Gtk2::Gdk::Pixbuf->new($pixbufs[0]->get_colorspace, 1, 8, $w, $h);
...
sub new {
    ...
    $icon = Gtk2::Image->new_from_stock($opt->{icon}, $opt->{size})
      if $opt->{icon};
    my $child =
      ($label && $icon)
      ? ::Hpack($icon, $label)
...
sub toggled_cb                          #also used by Layout::MenuItem
{
    ...
    if (my $tg = $self->{togglegroup}) {
        ...
        my @togbuttons =
          ...
          && $_ != $self
          && $_->{togglegroup} eq
          $tg,    #get list of widgets of the same togglegroup
          values %{$layw->{widgets}};
...
sub new {
    ...
    if ($opt->{updatemenu}) {
        ...
        ::IdleDo('9_UpdateSubMenu_' . $self, undef, \&UpdateSubMenu, $self)
          ; # (delayed) initial filling of the menu, not needed but makes the menu work better with gnome2-globalmenu
    }
    if ($opt->{togglewidget}) {
        ...
        ::Watch($self, 'HiddenWidgets',
            \&Layout::TogButton::UpdateToggleState);
    }
    if ($opt->{command}) {
...
sub new {
    ...
    $self->{menu}->signal_connect(deactivate =>
          sub { my $self = shift; $self->get_attach_widget->set_active(0); });
    return $self;
}
...
sub new {
    ...
    $self->signal_connect(size_allocate =>
          sub { ::IdleDo("resize_$self", 1000, \&update_columns, $_[0]); });
    return $self;
}
...
sub update_labels {
    ...
    for my $label (@{Songs::ListAll($self->{field})}) {
        my $check = $checks{$label} =
          Gtk2::CheckButton->new_with_label($label);
...
sub new {
    ...
    $self->{$_} = $opt->{$_}
      for
      qw/group ID markup_cat markup_field markup_value font expander collapsed hide_empty/;
...
sub init {
    ...
    while (@$treelist) {
        ...
        #category
        my $catlabel =
          Gtk2::Label->new_with_format($self->{markup_cat}, $catname);
        ...
        $cats->{$cat} = $catlabel;
        my $row = $table->{row}++;
        $table->attach($catlabel, 0, 1, $row, $row + 1, 'fill', 'shrink', 1,
            1);

        #fields
        for my $field (@$fields) {
            my $lab1 = $labels1->{$field} =
              Gtk2::Label->new_with_format($self->{markup_field},
        ...
        $row = $table->{row}++;
        $table->attach(Gtk2::HBox->new, 0, 3, $row, $row + 1, [], [], 0, 5)
          if @$treelist;    #space between categories
    }
...
sub update {
    ...
    $self->queue_resize
      ; # for unclear reasons (bug in gtk2 ?), labels that were hidden when the widget is was last shown, and then are shown while the widget is hidden, have a row height of 0 (and thus hidden) when the widget is shown, forcing a queue resize fixes it
}
...
sub SaveOptions {
    ...
    if (my $cats = $self->{cats}) {
        $opt{collapsed} = $self->{collapsed} = join ' ',
          sort grep !$cats->{$_}->get_expanded, keys %$cats;
    }
    return %opt;
}
...
our @ContextMenu = (
    ...
    {   label => sub {
        ...
        code =>
          sub { Songs::Picture($_[0]{gid}, $_[0]{field}, 'set', $_[0]{file}); }
        ,
        test => sub {
            $_[0]{file} ne
              (Songs::Picture($_[0]{gid}, $_[0]{field}, 'get') || '');
        },
        istrue => 'file gid',
        mode   => 'V',
    ...
    {   label => "Paste link",
        test  => sub {
            ...
            $_[0]{clip} =
              $_[0]{self}->get_clipboard(Gtk2::Gdk::Atom->new('PRIMARY', 1))
              ->wait_for_text;
            $_[0]{clip} && $_[0]{clip} =~ m#^\s*\w+://#;
        },
        code => sub {
            $_[0]{self}->drop_uris(
                uris => [grep m#^\s*\w+://#, split /[\n\r]+/, $_[0]{clip}]);
        },
    },
    ...
    {   label =>
        ...
        mode      => 'VP',
        stockicon => sub {
            $_[0]{view}{fullwin} ? 'gtk-leave-fullscreen' : 'gtk-fullscreen';
        },
    },
...
sub new {
    ...
    $self->{$_} = $opt->{$_}
      for
      qw/group follow show_list show_folders show_toolbar reset_zoom_on nowrap pdf_mode embedded_mode all_mode/;

    my $hbox = Gtk2::HBox->new;
    my $hpaned =
      Layout::Boxes::PanedNew('Gtk2::HPaned', {size => $opt->{hpos}});
    my $vpaned = $self->{vpaned} =
      Layout::Boxes::PanedNew('Gtk2::VPaned', {size => $opt->{vpos}});
    my $view = $self->{view} =
      Layout::PictureBrowser::View->new(%$opt, mode => 'V');
    my $toolbar = $self->{toolbar} =
      ::BuildToolbar(\@toolbar, getcontext => \&toolbarcontext, self => $self);
    $self->{dirstore} =
      Gtk2::ListStore->new(qw/Glib::String Glib::String Glib::String/);
    $self->{filestore} = Gtk2::TreeStore->new(
        qw/Glib::String Glib::String Glib::Boolean Glib::Uint Glib::Uint/);
    ...
    $treeview1->insert_column_with_attributes(-1, 'Dir name',
        Gtk2::CellRendererText->new,
        text => 0);
    $treeview2->insert_column_with_attributes(
        -1, 'File name', $renderer,
        text     => 0,
        editable => 2
    );
    ...
    my $column_s =
      Gtk2::TreeViewColumn->new_with_attributes('size', $renderer_s);
    my $column_d =
      Gtk2::TreeViewColumn->new_with_attributes('date', $renderer_d);
    ...
    $treeview2->signal_connect_after(
        expose_event => sub {
            ...
            if ($rect->intersect($event->area)) {
                my $layout =
                  Gtk2::Pango::Layout->new($tv->create_pango_context);
                ...
                if ($self->{embfiles_idle})    #draw progress line
                {
                    ...
                    $tv->get_bin_window->draw_rectangle($gc, ::TRUE, 0,
                        $rect->y, $width, 2);
                }
                $layout->set_markup($tv->{embfiles_text});
                $tv->get_style->paint_layout($tv->get_bin_window, $state, 1,
                    $rect,    $tv, undef, 4 + $rect->x,
                    $rect->y, $layout);
            }
        }
    );
...
sub SaveOptions {
    ...
    $opt{$_}   = $self->{$_}
      for
      qw/follow show_list show_folders show_toolbar reset_zoom_on pdf_mode embedded_mode all_mode/;
    return %opt;
}
...
sub filename_edited_cb {
    ...
    my $new =
      GMB::Picture::RenameFile($path, $file, $newutf8, $tv->get_toplevel)
      if $newutf8 =~ m/\S/ && $file ne $newutf8;
    return unless $new;
...
sub folder_button_press_cb {
    my ($tv, $event) = @_;
    return 1
      if $event->type ne 'button-press';    # ignore double or triple clicks
...
sub file_button_press_cb {
    ...
    if ($button == 3) {
        ...
        ::PopupContextMenu(\@ContextMenu,
            {mode => 'L', treepaths => \@rows, $self->context_menu_args});
    }
    else { return 0 }
    1;
}
...
sub SongChanged {
    ...
    $self->{view}->reset_zoom
      if $self->{reset_zoom_on} eq 'group' && $oldgid == $self->{gid};
    $self->update;
}
...
sub update {
    ...
    if ($self->{mode} eq 'song') {
        my $field = $self->{field};
        my $gid   = $self->{gid};
        @emb_files = Songs::Map('fullfilename', AA::GetIDs($field, $gid))
          if $emb_ok;
        ...
        @files =
          grep { my $f = $_; $f =~ s/:\w+$//; -f $f } @{$self->{filelist}};
    }

    my $pdfok = $GMB::Picture::pdf_ok && $self->{pdf_mode};
    for my $path (@paths) {
        opendir my ($dh), $path or do { warn $!; last; };
        for my $file (map $path . ::SLASH . $_,
            ::sort_number_aware(grep !m#^\.#, readdir $dh))
        {
            next if -d $file;
            if ($file =~ m/$::Image_ext_re/) { push @files, $file }
            elsif ($file =~ m/\.pdf$/i && $pdfok) {
                push @files, $file, map "$file:$_",
                  1 .. GMB::Picture::pdf_pages($file) - 1;
            }
            else {
    ...
    $self->{current_path} = $path;
    $self->{view}->reset_zoom
      if $self->{reset_zoom_on} eq 'folder'
      && $oldpath ne $self->{current_path};
    $self->refresh_treeviews;
    $self->update_file;
}
...
sub refresh_treeviews {
    ...
    unless ($path
        && $oldpath
        && $oldpath eq $path)    #folder has changed, reset scrollbars
    {
    ...
    $dirstore->set($dirstore->append, 0, '..', 1,
        Songs::filename_escape($parent),
        2, 'gtk-go-up')
      if $parent;
    my $pdfok = $GMB::Picture::pdf_ok && $self->{pdf_mode};
    ...
    for my $file (::sort_number_aware(grep !m#^\.#, readdir $dh)) {
        if (-d $path . $file) {
            ...
            $dirstore->set(
                $iter, 0, ::filename_to_utf8displayname($file),
                1,     Songs::filename_escape($path . $file),
                2,     'gtk-directory'
            );
            if (   $oldpath
                && $oldpath eq $path
                . $file)    #select and center on previous folder if there
            {
        ...
        $filestore->set($iter, 0, ::filename_to_utf8displayname($file),
            1, $efile, 2, !$::CmdLine{ro}, 3, $size, 4, $time);
        for my $n (@pages) {
            $filestore->set(
    ...
    if ($self->{embfiles}) {
        ...
        $self->scan_embedded_pictures_update_tv
          unless $self->{embfiles}{updatetv}
          ;    # if scan of embedded files was made immediately
    }

    $filetv->set_show_expanders($show_expanders);
    $filetv->expand_all;
    $foldertv->scroll_to_cell($folder_center_treepath, undef, ::TRUE, .5, 0)
      if $folder_center_treepath;    #needs to be done once the list is filled
}
...
sub scan_embedded_pictures {
    ...
    if ($now) { $now = $self->scan_embedded_pictures_idle_cb while $now; }
        ...
        $self->{embfiles_idle}
          ||= Glib::Idle->add(\&scan_embedded_pictures_idle_cb, $self);
    }
}
...
sub scan_embedded_pictures_idle_cb {
    ...
    for my $p (0 .. $#p) {
        next unless $p[$p];
        my $i = ::first {$p[$p] eq $pix->[$_][1]}
          0 .. $#$pix;    #check if already seen that picture
        unless (defined $i) {
            $i = @$pix;
            push @$pix, [$file . ':' . $p, $p[$p]];
        }                 #new picture
    ...
    $self->scan_embedded_pictures_update_tv
      if $self->{embfiles}{updatetv} && $self->{show_list};
...
sub update_file {
    ...
    my $pixbuf = $file
      && GMB::Picture::pixbuf($file, undef, undef, 'anim_ok'); #disable cache ?
    my %info;
    if ($file) {
        my $realfile = $file;
...
sub update_selection {
    ...
    while ($iter) {
        if ($store->get($iter, 1) eq $file) {
            ...
            $treeview->scroll_to_cell($store->get_path($iter),
                undef, ::FALSE, 0, 0);    #scroll to row if needed
            last;
        }
...
sub context_menu_args {
    ...
    my $file = $self->{current_file};
    my $pixi =
      $file && $self->{embfiles} && $self->{embfiles}{file2ref}{$file};
    my $embfiles = $pixi
      && $pixi->[2];    #list of file:n that have the selected picture in tag
...
sub new {
    ...
    $self->add_events(
        [   qw/pointer-motion-mask scroll-mask key-press-mask button-press-mask button-release-mask/
        ]
    );
    $self->can_focus(::TRUE);
    $self->signal_connect(expose_event         => \&expose_cb);
...
sub set_pixbuf {
    ...
    $self->{pixbuf} = $pixbuf;
    delete $self->{pbanim};
    Glib::Source->remove(delete $self->{anim_timeout})
      if $self->{anim_timeout};
...
sub animate {
    ...
    $self->{anim_timeout} = Glib::Timeout->add($ms, \&animate, $self)
      if $ms > 0;
    0;
}
...
sub button_press_cb {
    ...
    if ($button == 3) {
        ...
        push @args, $parent->context_menu_args
          if $parent && $parent->can('context_menu_args');
...
sub button_release_cb {
    ...
    if (($self->{pressed} || 0) == $button) {
        ...
        $self->{$_} = undef
          for qw/last_x last_y dragged pressed zoomed prevnext scrolled/;
    }
    else { return 0 }
    1;
}
...
sub motion_notify_cb {
    ...
    if ($button == 1 && ($self->{max_x} || $self->{max_y})) {
        $event->window->set_cursor(Gtk2::Gdk::Cursor->new('fleur'))
          unless $self->{dragged};
        $self->{dragged} = 1;
      ...
      ) # zoom or prev/next with other button # only prev/next once by button press
    {
        ...
        my $next =
          int(($self->{last_x} - $ex) / 30);    # horizontal movement >=30
        if ($zoom) {
            $zoom = "+$zoom" if $zoom > 0;
            $self->change_zoom($zoom, $ex, $ey);
            $self->{last_y} = $ey;
            $event->window->set_cursor(Gtk2::Gdk::Cursor->new('double_arrow'))
              unless $self->{zoomed};
            $self->{zoomed} = 1;
        }
...
#my $mod=  $state * [qw/control-mask mod1-mask mod4-mask super-mask/]; # no modifier ctrl/alt/super
    ...
    if (   $updown
        && !$ctrl
        && ($shift || (!$self->{scroll_zoom} xor $button1))
...
sub invalidate_gdkwin {
    ...
    $gdkwin->invalidate_rect(
        Gtk2::Gdk::Rectangle->new(0, 0, $gdkwin->get_size), 0)
      if $gdkwin;
}
...
sub set_fullscreen {
    ...
    if ($fullscreen) {
        ...
        my %attr = (
            ...
            event_mask  => [
                qw/exposure-mask pointer-motion-mask button-press-mask button-release-mask key-press-mask/
            ],
        );
    ...
    $self->{pressed} = undef
      ; #reset mouse state, in particular when using double middle button to fullscreen, the mouse could stay in middle-button-pressed mode
    $self->resize;
}
...
sub click {
    ...
    $x = 1 if $x > 1;
    my $pb =
      $Songs::Def{$self->{field}}{pixbuf} || $Songs::Def{'rating'}{pixbuf};
    my $nbstars = $#$pb;
...
use constant SEPARATOR => '  '
  ; # must not be a possible name of a preset, use "  " because EqualizerPresets won't let you create names that contain only spaces
...
sub update {
    ...
    my $full =
      ...
      || $event eq 'package'
      || $event eq
      'presetlist';    #first time or package changed or preset_list changed
...
sub update_buttons {
    ...
    if ($self->{entry}) {
        ...
        my $ok  = $new =~ m/\S/
          && ($self->{changed} || !exists $self->{presets}{$new});
        $self->{sbutton}->set_sensitive($ok);
        my $current = $self->{combo}->get_active_text;
        $self->{rbutton}->set_sensitive(defined $current
              && exists $self->{presets}{$current});
    }
}
...
sub new {
    ...
    if ($opt->{preamp}) {
        ...
        $adj->signal_connect(
            value_changed => sub {
                ::SetEqualizer(preamp => $_[0]->get_value)
                  unless $_[0]{busy};
            }
        );
        if ($self->{labels}) {
...
sub update {
    ...
    if ($doall || $event eq 'active') {
        ...
        $self->{preamp_widget}->set_sensitive($ok && $::Play_package->{EQpre})
          if $self->{preamp_widget};
    }
    if ($doall && $self->{labels}) {
        ...
        for my $i (0 .. 9) {
            if ($self->{labels}) {
                my $val = '-';
                $val = $::Play_package->EQ_Get_Hz($i) || '?'
                  if $::Play_package->{EQ};
                $self->{'Hzlabel' . $i}
                  ->set_markup_with_format(qq(<span size="%s">%s</span>),
                    $self->{labels}, $val);
            }
            my $adj = $self->{'adj' . $i};
            $adj->{busy} = 1;
    ...
    if ($doall || $event eq 'values') {
        ...
        for my $i (0 .. 9) {
            ...
            $self->{'Valuelabel' . $i}
              ->set_markup_with_format(qq(<span size="%s">%.1f%s</span>),
                $self->{labels}, $val, $self->{unit});
        }
    }
    if (($doall || $event eq 'preamp') && (my $adj = $self->{adj_preamp})) {
        ...
        $adj->set_value($val);
        delete $adj->{busy};
        $self->{Valuelabel_preamp}
          ->set_markup_with_format(qq(<span size="%s">%d%%</span>),
            $self->{labels}, ($val**3) * 100)
          if $self->{Valuelabel_preamp};
    }
}
...
use Glib::Object::Subclass
  Gtk2::VBox::,
  signals => {size_allocate => \&SHBox::size_allocate,};
...
sub size_allocate {
    ...
    elsif ($total_xreq < $bwidth
        && $ecount)    # if enough room for all, and some have expand attribute
    {
        ...
        while ($ecount && $w > $ecount) {
            ...
            for
              my $child (grep $_->[1], @children) #children that want to expand
            {
                my $max    = $child->[6];
                my $expand = $child->[1];
        ...
        {
            for my $child (sort { $b->[1] <=> $a->[1] } grep $_->[1],
                @children)        # start with highest weight
            {
...
# note that weightsum, ecount, max and expand are not updated here as they are no longer used
        ...
        && @children)                #not enough room for requested width
    {
        my $w = $bwidth - $total_spacing;

        # [5] is request [3] is padding
        my @tofit = sort { $a->[5] + $a->[3] * 2 <=> $b->[5] + $b->[3] * 2 }
          @children;                 # sort from smallest to largest
        while (my $child = shift @tofit) {
            my $give =
              int($w / (1 + @tofit));    # space available for each child left
            $give = 0 if $give < 0;
...
sub new {
    ...
    my $self = bless {}, $class;
    ($self->{file}, $self->{crop}, $self->{resize}, my $states1, my $states2)
      = split /:/, $string;
    my %states;
    my @states2 =
      $states2
...
sub draw {
    ...
    $gc->set_clip_rectangle($event->area);
    $widget->window->draw_pixbuf($gc, $pb, 0, 0, $x, $y, $pbw, $pbh, 'none',
        0, 0);
    $style->paint_focus(
        $widget->window, $state1, $event->area, $widget, undef,
        $x,              $y,      $pbw,         $pbh
    ) if $widget->has_focus;
...
sub makepixbuf {
    my ($self, $state, $w, $h) = @_;
    my $pb =
      _load_skinfile($self->{file}, $self->{crop}, $self->{skin_options});
    return undef unless $pb;
    if (my $states = $self->{states}) {
...
sub _resize {
    ...
    my $dest =
      Gtk2::Gdk::Pixbuf->new($src->get_colorspace, $src->get_has_alpha,
        $src->get_bits_per_sample, $width, $height);

    #4 corners
    $src->copy_area(0, 0, $left, $top, $dest, 0, 0) if $left && $top;
    $src->copy_area($w - $right, 0, $right, $top, $dest, $width - $right, 0)
      if $right && $top;
    $src->copy_area(0, $h - $bottom,
        $left, $bottom, $dest, 0, $height - $bottom)
      if $left && $bottom;
    $src->copy_area(
        $w - $right,
        $h - $bottom,
    ...
    push @parts,
      [ $left,   $h - $bottom, $wi,
        $bottom, $left,        $height - $bottom,
        $dwi,    $bottom,      $s_b
      ]
      if $bottom;
    push @parts, [0, $top, $left, $hi, 0, $top, $left, $dhi, $s_l] if $left;
    push @parts,
      [ $w - $right, $top,   $right, $hi, $width - $right,
        $top,        $right, $dhi,   $s_r
      ]
      if $right;

    #center
    push @parts, [$left, $top, $wi, $hi, $left, $top, $dwi, $dhi, $s_c]
      if $hi && $wi;
        ...
        else              #repeat to cover the area
        {
            ...
            for my $x3 (map $w * $_, 0 .. int($w2 / $w) + 1) {
                ...
                for my $y3 (map $h * $_, 0 .. int($h2 / $h) + 1) {
                    ...
                    next unless $h1;
                    $subp->copy_area(0, 0, $w1, $h1, $dest, $x2 + $x3,
                        $y2 + $y3);
                }
            }
        }
