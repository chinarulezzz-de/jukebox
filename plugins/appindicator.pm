...
sub Update {
    ...
    my ($menuentry) =
      grep $_->{id} && $_->{id} eq $::Options{OPT . 'MiddleClick'},
      $menu->get_children;
...
sub UpdateIcon {
    my $state =
      !defined $::TogPlay ? 'default' : $::TogPlay ? 'play' : 'pause';
    $state = 'default' unless $::TrayIcon{$state};
    my $path = ::dirname($::TrayIcon{$state});
    my $name = ::barename($::TrayIcon{$state});
    $indicator->set_icon_theme_path($iconpath = $path)
      if $iconpath && $iconpath ne $path;
