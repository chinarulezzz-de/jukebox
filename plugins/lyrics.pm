...
my %Sites
  = # id => [name,url,?post?,function]	if the function return 1 => lyrics can be saved
...
#		sub { my $ok=$_[0]=~m#<div id="lyrics">.*\w\n.*\w.*</div>#s; $_[0]=~s/<div id="admin".*$//s if $ok; return $ok; }],
    lyricssongs => [
        ...
        sub {
            ...
            $l =~ s#<div id="cabecalho">.*?</div>##s
              if $l;    #remove header with title and artist links
            my $ref = \$_[0];
            $$ref = $l ? $l : $notfound;
...
$::Options{OPT . 'Font'}
  ||= delete $::Options{OPT . 'FontSize'};    #for versions <1.1.6
...
sub SongChanged {
    ...
    $self
      ->cancel;  #cancel any lyrics operation in progress on an a previous song
    $self->{ID}   = $ID;
    $self->{time} = undef;
    ...
    if (my $url = $self->url_at_coords($x, $y)) {
        ...
        $item3->signal_connect(
            activate => sub {
                my $url = $_[1];
                my $clipboard =
                  $_[0]->get_clipboard(Gtk2::Gdk::Atom->new('CLIPBOARD', 1));
            ...
            if ($loader) {
                ...
                if ($link) {
                    ...
                    $buffer->apply_tag($tag,
                        $buffer->get_iter_at_offset($offset), $iter);
                }
            }
...
BEGIN {
    %zoomkeymap = (plus => 1, minus => -1, KP_Add => 1, KP_Subtract => -1);
}
...
sub load_from_file {
    ...
    my $ID   = $self->{ID};
    my $text =
      $::Options{OPT . 'PreferEmbeddedLyrics'}
