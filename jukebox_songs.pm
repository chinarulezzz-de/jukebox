...
INIT {
    ...
    %Types = (
        generic => {
            ...
            'filter:fuzzy' =>
              '.!!. Filter::_fuzzy_match(#VAL1#/100,"#VAL2#",lc(#get#))',
        ...
        flags   => {
            ...
            get_list =>
              'my $v=#_#; ref $v ? map(___name[$_], @$v) : $v ? ___name[$v] : ();',
            ...
            get =>
              'do {my $v=#_#; !$v ? "" : ref $v ? join "\\x00",map ___name[$_],@$v : ___name[$v];}',
            newval =>
              'push @___iname, ::superlc(___name[-1]); ::IdleDo("newgids_#field#",1000,sub {  ___new=0; ::HasChanged("newgids_#field#"); }) unless ___new++;',
            sgid_to_gid =>
              '___gid{#VAL#}||= do { my $i=push(@___name, #VAL#); #newval#; $i-1; }',
            set => '{my $v=#VAL#;
            ...
            diff =>
              'do {my $v=#_#; my $old=!$v ? "" : ref $v ? join "\\x00",map ___name[$_],@$v : ___name[$v]; $v=#VAL#; my $new= join "\\x00", @$v; $old ne $new; }'
            ,    # #VAL# should be a sorted arrayref, as returned by #check#
            display =>
              'do { my $v=#_#; !$v ? "" : ref $v ? join ", ",map ___name[$_],@$v : ___name[$v]; }',
            check_multi =>
              'for my $lref (@{#VAL#}) { for (@$lref) {tr/\x00-\x1F//d; s/\s+$//;} }',
            set_multi =>
              'do {my $c=#_#; my %h=( $c ? ref $c ? map((___name[$_]=>0), @$c) : (___name[$c]=>0) : ()); my ($toadd,$torm,$toggle)=@{#VAL#}; $h{$_}= (exists $h{$_} ? -1 : 1) for @$toggle; $h{$_}++ for @$toadd; $h{$_}-- for @$torm; (scalar grep $h{$_}!=0, keys %h) ? [grep $h{$_}>=0, keys %h] : undef; }',
            makefilter =>
              '#GID# ? "#field#:~:".___name[#GID#] : "#field#:ecount:0"',
            'filter:~' =>
              '.!!. do {my $v=#_#; $v ? ref $v ? grep(#VAL#==$_, @$v) : ($v == #VAL#) : 0}',

#smartmatch version: 'filter:~'	=> '.!!. do {my $v=#_#; $v ? #VAL# ~~ $v : 0}', # is flag set
            'filter_prep:~' => '___gid{#PAT#} ||= #sgid_to_gid(VAL=#PAT#)#;',
            'filter_prephash:~' =>
              'return { map { #sgid_to_gid(VAL=$_)#, undef } keys %{#HREF#} }',
            'filter:h~' =>
              '.!!. do {my $v=#_#; $v ? ref $v ? grep(exists $hash#VAL#->{$_+0}, @$v) : (exists $hash#VAL#->{#_#+0}) : 0}',
            'filter:ecount' =>
              '#VAL# .==. do {my $v=#_#; $v ? ref $v ? scalar(@$v) : 1 : 0}',

#FIXME for filters s,m,mi,h~,  using a list of matching names in ___inames/___names could be better (using a bitstring)
            'filter:s' =>
              'do { my $v=#_#; !$v ? .0. : ref $v ? (.!!. grep index(___name[$_], "#VAL#")  != -1 ,@$v) : (index(___name[$v], "#VAL#")  .!=. -1); }',
            'filter:si' =>
              'do { my $v=#_#; !$v ? .0. : ref $v ? (.!!. grep index(___iname[$_], "#VAL#") != -1 ,@$v) : (index(___iname[$v], "#VAL#") .!=. -1); }',
            'filter:fuzzy' =>
              'do { my $v=#_#; !$v ? .0. : ref $v ? (.!!. ::first {Filter::_fuzzy_match(#VAL1#/100,"#VAL2#",___iname[$_])} @$v) : .!!. Filter::_fuzzy_match(#VAL1#/100,"#VAL2#",___iname[$v]); }',
            'filter:m' =>
              'do { my $v=#_#; !$v ? .0. : ref $v ? (.!!. grep ___name[$_]  =~ m"#VAL#"  ,@$v) : ___name[$v]  .=~. m"#VAL#"; }',
            'filter:mi' =>
              'do { my $v=#_#; !$v ? .0. : ref $v ? (.!!. grep ___iname[$_] =~ m"#VAL#"i ,@$v) : ___iname[$v] .=~. m"#VAL#"i; }',
            'filter_prep:m'  => \&Filter::QuoteRegEx,
            'filter_prep:mi' => sub { Filter::QuoteRegEx(::superlc($_[0])) },
            'filter_prep:si' => sub { quotemeta ::superlc($_[0]) },
            'filter_prep:s'  => sub { quotemeta $_[0] },
            ...
            stats =>
              'do {my $v=#_#; #HVAL#{$_+0}=undef for ref $v ? @$v : $v;}  ---- AFTER: #HVAL#=[map ___name[$_], keys %{#HVAL#}];',
            'stats:gid' =>
              'do {my $v=#_#; #HVAL#{$_+0}=undef for ref $v ? @$v : $v;}',
            hashm => 'do {my $v=#_#; ref $v ? @$v : $v }',
            'hashm:name' =>
              'do {my $v=#_#; ref $v ? map(___name[$_], @$v) : $v ? ___name[$v] : () }',
            is_set =>
              'my $gid=___gid{#VAL#}; my $v=#_#; $gid ? ref $v ? (grep $_==$gid, @$v) : $v==$gid : 0;',
...
#smartmatch version : is_set	=> 'my $gid=___gid{#VAL#}; my $v=#_#; $gid ? $gid ~~ $v : 0;',
            ...
            save_extra =>
              'my %h; while ( my ($sgid,$gid)=each %___gid ) { $h{$sgid}= [#SUBFIELDS#] } delete $h{""}; return \%h;',
...
#		_iname		=> '__#mainfield#_iname[#_#]',
            get =>
              'do {my $v=#_#; ref $v ? join "\\x00",map __#mainfield#_name[$_],@$v : __#mainfield#_name[$v];}',
            display =>
              'do {my $v=#_#; ref $v ? join ", ",   map __#mainfield#_name[$_],@$v : __#mainfield#_name[$v];}',
            get_gid       => 'my $v=#_#; ref $v ? $v : [$v]',
            ...
            get_list =>
              'my @l=( ##mainfield#->get#, grep(defined, #title->get# =~ m/$Artists_title_re/g) ); my %h; grep !$h{$_}++, map split(/$Artists_split_re/), @l;',
            gid_to_get => '(#GID#!=1 ? __#mainfield#_name[#GID#] : "")'
            ,    # or just '__#mainfield#_name[#GID#]' ?
            ...
            'filter:m' =>
              '(ref #_# ?  (.!!. grep __#mainfield#_name[$_]  =~ m"#VAL#",  @{#_#}) : (__#mainfield#_name[#_#]  .=~. m"#VAL#"))',
            'filter:mi' =>
              '(ref #_# ?  (.!!. grep __#mainfield#_iname[$_] =~ m"#VAL#"i, @{#_#}) : (__#mainfield#_iname[#_#] .=~. m"#VAL#"i))',
            'filter:s' =>
              '(ref #_# ?  (.!!. grep index( __#mainfield#_name[$_],"#VAL#") != -1, @{#_#}) : (index(__#mainfield#_name[#_#],"#VAL#") .!=. -1))',
            'filter:si' =>
              '(ref #_# ?  (.!!. grep index( __#mainfield#_iname[$_], "#VAL#") != -1, @{#_#}) : (index(__#mainfield#_iname[#_#], "#VAL#") .!=. -1))',
            'filter:fuzzy' =>
              'do { my $v=#_#; ref $v ? (.!!. ::first {Filter::_fuzzy_match(#VAL1#/100,"#VAL2#",__#mainfield#_iname[$_])} @$v) : .!!. Filter::_fuzzy_match(#VAL1#/100,"#VAL2#",__#mainfield#_iname[$v]); }',
            'filter_prep:m'  => \&Filter::QuoteRegEx,
            'filter_prep:mi' => sub { Filter::QuoteRegEx(::superlc($_[0])) },
            'filter_prep:si' => sub { quotemeta ::superlc($_[0]) },
            'filter_prep:s'  => sub { quotemeta $_[0] },
            ...
            'filter_simplify:~' =>
              sub { length($_[0]) ? split /$Artists_split_re/, $_[0] : $_[0]; }
            ,
            'filter:h~' =>
              '(ref #_# ?  (grep .!!. exists $hash#VAL#->{$_+0}, @{#_#}) : (.!!. exists $hash#VAL#->{#_#+0}))',
            makefilter => '"#field#:~:".##mainfield#->gid_to_sgid#',

            #group		=> '#_# !=',
            stats =>
              'do {my $v=#_#; #HVAL#{__#mainfield#_name[$_]}=undef for ref $v ? @$v : $v;}  ---- AFTER: #HVAL#=[keys %{#HVAL#}];',
            'stats:gid' =>
              'do {my $v=#_#; #HVAL#{$_}=undef for ref $v ? @$v : $v;}  ---- AFTER: #HVAL#=[keys %{#HVAL#}];',
            hashm   => 'do {my $v=#_#; ref $v ? @$v : $v}',
        ...
        artist =>
          { #set		=> '#_#= (#VAL# eq "" ? 0 : (__#mainfield#_gid{#VAL#}||= (push @__#mainfield#_name, #VAL#)-1));',
            ...
            search_gid =>
              'my $gid=__#mainfield#_gid{#VAL#}||0; $gid>1 ? $gid : undef;',
            makefilter => '"#field#:~:" . #gid_to_sgid#',
            diff =>
              'do {my $old=#_#; ($old!=1 ? #_name#[$old] : "") ne #VAL# }',
...
#save_extra	=> 'my %h; for my $gid (2..$##_name#) { my $v=__#mainfield#_picture[$gid]; next unless defined $v; ::_utf8_on($v); $h{ #_name#[$gid] }=$v; } return artist_pictures',
        ...
        album => {
            ...
            load => '#_#= #findgid#;',
            set =>
              'my $oldgid=#_#; my $newgid= #_#= #findgid#; if ($newgid+1==@#_name# && $newgid!=$oldgid) { ___picture[$newgid]= ___picture[$oldgid]; }'
            , #same as load, but if gid changed and is new, use picture from old gid

            #newval		=> 'push @#_iname#, ::superlc( #_name#[-1] );',
            get => '(#_empty# ? "" : #_name#[#_#])',
            gid_to_get =>
              '(vec(__#mainfield#_empty,#GID#,1) ? "" : #_name#[#GID#])',
            sgid_to_gid =>
              'do {my $s=#VAL#; __#mainfield#_gid{$s}||= do { my $n=@#_name#; if ($s=~s/\x00(\w+)=(.*)$// && $s eq "" && $1 eq "artist") { $s= #unknown#.$2; vec(__#mainfield#_empty,$n,1)=1;} push @#_name#,$s; push @__#mainfield#_sgid,#VAL#; #newval#; $n }}',
            ...
            'stats:artistsort' =>
              '#HVAL#->{ #album_artist->get_gid# }=undef;  ---- AFTER: #HVAL#=do { my @ar= keys %{#HVAL#}; @ar>1 ? ::superlc(_"Various artists") : __artist_iname[$ar[0]]; }',
...
#save_extra	=> 'my @res; for my $gid (1..$##_name#) { my $v=___pix[$gid]; next unless length $v; push @res, [#*:gid_to_sgid(GID=$gid)#,$val]; } return \@res;',
        ...
        string => {
            parent  => 'generic',
            default => '""',
            check   => '#VAL#=~tr/\x1D\x00//d; #VAL#=~s/\s+$//;'
            ,    #remove trailing spaces and \x1D\x00
            diff       => '#_# ne #VAL#',
            s_sort     => '#_#',
            'filter:e' => '#_# .eq. "#VAL#"',
            hash       => '#_#',
            group      => '#_# ne',
            stats =>
              '#HVAL#{#_#}=undef;  ---- AFTER: #HVAL#=[keys %{#HVAL#}];',
        },
        istring => # _much_ faster with case/accent insensitive operations, at the price of double memory
          { parent           => 'string',
            ...
            'filter_prep:mi' => sub { Filter::QuoteRegEx(::superlc($_[0])) },
            'filter:fuzzy' =>
              ' .!!. Filter::_fuzzy_match(#VAL1#/100,"#VAL2#",#_iname#)',
            'filter_prep:fuzzy' => sub {
                my @arg = split / /, $_[0], 2;
                $arg[0], quotemeta ::superlc($arg[1]);
            },
        ...
        text =>    #multi-lines string
          { parent => 'string',
            check =>
              '#VAL#=~tr/\x00-\x09\x0B\x0C\x0E-\x1F//d; #VAL#=~s/\s+$//;',
        ...
        filename => {
            parent => 'string',
            check  => ';'
            , #override string's check because not needed and filename may not be utf8
            get             => '#_#',
            set             => '#_#=#VAL#; ::_utf8_off(#_#);',
            display         => '::filename_to_utf8displayname(#get#)',
            hash_to_display => '::filename_to_utf8displayname(#VAL#)'
            ,    #only used by FolderList:: and MassTag::
...
#	},
        _picture => {
            ...
            init =>
              '@__#mainfield#_picture=(); push @GMB::Picture::ArraysOfFiles, \@__#mainfield#_picture;',
            default     => '$::Options{Default_picture}{#mainfield#}',
            get_for_gid => '#_# || #default#;',
            pixbuf_for_gid =>
              'my $file= #get_for_gid#; GMB::Picture::pixbuf($file);',
            set_for_gid =>
              '::_utf8_off(#VAL#); #_#= #VAL# eq "" ? undef : #VAL#; ::HasChanged("Picture_#mainfield#",#GID#);',
            load_extra => 'if (#VAL# ne "") { #_#= ::decode_url(#VAL#); }',
            save_extra =>
              'do { my $v=#_#; defined $v ? filename_escape($v) : ""; }',
        ...
        fewstring =>    #for strings likely to be repeated
          { _    => 'vec(____,#ID#,#bits#)',
            ...
            sgid_to_gid =>
              '__#mainfield#_gid{#VAL#}||= do { my $i=push(@#_name#, #VAL#); #newval#; $i-1; }',
            newval => 'push @#_iname#, ::superlc( #_name#[-1] );',

            #newval		=> 'push @#_iname#, ::superlc(#VAL#);',
            set => '#_# = #sgid_to_gid#;',
            init =>
              '____=""; __#mainfield#_gid{""}=1; #_name#[1]=#_iname#[1]="";',
            ...
            'filter:fuzzy' =>
              '.!!. Filter::_fuzzy_match(#VAL1#/100,"#VAL2#",#_iname#[#_#])',
            'filter_prep:fuzzy' => sub {
                my @arg = split / /, $_[0], 2;
                $arg[0], quotemeta ::superlc($arg[1]);
            },
...
#		hash		=> '#_name#[#_#]',
            ...
            group      => '#_# !=',
            stats =>
              '#HVAL#{#_name#[#_#]}=undef;  ---- AFTER: #HVAL#=[keys %{#HVAL#}];',
            'stats:gid' =>
              '#HVAL#{#_#}=undef;  ---- AFTER: #HVAL#=[keys %{#HVAL#}];',
            listall         => '2..@#_name#-1',
            edit_listall    => 1,
...
#gsummary	=> 'my $gids=Songs::UniqList(#field#,#IDs#); @$gids==1 ? #gid_to_display(GID=$gids->[0])# : #names(count=scalar @$gids)#;',
        ...
        number => {
            ...
            'group'         => '#_# !=',
            'stats:range' =>
              'push @{#HVAL#},#_#;  ---- AFTER: #HVAL#=do {my ($m0,$m1)=(sort {$a <=> $b} @{#HVAL#})[0,-1]; $m0==$m1 ? $m0 : "$m0 - $m1"}',
            'stats:average' =>
              'push @{#HVAL#},#_#;  ---- AFTER: #HVAL#=do { my $s=0; $s+=$_ for @{#HVAL#}; $s/@{#HVAL#}; }',
            'stats:sum' => '#HVAL# += #_#;',
            stats       => '#HVAL#{#_#+0}=undef;',
            hash        => '#_#+0',
            display     => '(#_# ? sprintf("#displayformat#", #_# ) : "")'
            ,    #replace 0 with ""
            gid_to_display => '#GID#',
            get_gid        => '#_#+0',
...
#makefilter	=> '"#field#:".(!#GID# ? "e:0" : "b:".(#GID# * #ARG0#)." ".((#GID#+1) * #ARG0#))',
            makefilter =>
              'Filter->newadd(1, "#field#:-<:".(#GID# * #ARG0#), "#field#:<:".((#GID#+1) * #ARG0#) )'
        ...
        fewnumber => {
            ...
            set =>
              'vec(____,#ID#,#bits#) = ___gid{#VAL#}||= do { push(@___value, #VAL#+0)-1; }',
        ...
        integer => {
            ...
            check => '#VAL#= #VAL# =~m/^(\d+)/ && $1<2**#bits# ? $1 : 0;'
            ...
            step =>
              1, #minimum difference between 2 values, used to simplify filters
        },
        'integer.div' => {
            makefilter =>
              '"#field#:b:".(#GID# * #ARG0#)." ".(((#GID#+1) * #ARG0#)-1)',
        },
        float => #make sure the string doesn't have the utf8 flag, else substr won't work
          { _ => 'unpack("F",substr(____,#ID#<<3,8))',
            display =>
              'do {my $v=#_#; (#v_is_nan# ? "" : ::format_number($v,"#displayformat#"))}'
            ,    # replace novalue (NaN) with ""
            ...
            parent        => 'number',
            nan           => '$Songs::nan',
            v_is_nan => ($nan == $nan ? '($v==#nan#)' : '($v!=$v)')
            , #on some system $nan!=$nan, on some not. In case nan==0, 0 will be treated as novalue, could treat novalue as 0 instead
            novalue => '#nan#',       #use NaN as novalue
            default => '#novalue#',
            set =>
              'substr(____,#ID#<<3,8)=pack("F",(length(#VAL#) ? #VAL# : #novalue#))',
            check =>
              '#VAL#= #VAL# =~m/^(-?\d*\.?\d+(?:e[-+]\d+)?)$/i ? $1 : #novalue#;',
...
# FIXME make sure that locale is set to C (=> '.' as decimal separator) when needed
            ...
            , #hval=nan if $v1!=$v2 works both if nan==nan or nan!=nan : set hval to nan if either one of them is nan or if they are not equal. That way no need to use #v_is_nan#, which would be complicated as it uses $v
          },
        'float.range' => {
            get_gid =>
              'do {my $v=#_#; #v_is_nan# ? #nan_gid# : int($v/#range_step#) ;}',
            nan_gid =>
              '-2**31+1',    #gid in FilterList are Long, 2**31-1 is GID_ALL
            always_first_gid => -2**31 + 1,
            range_step       => '1',          #default step
            gid_to_display =>
              '( #GID#==#nan_gid# ? _"not defined" : do {my $v= #GID# * #range_step#; "$v .. ".($v+#range_step#)})',
            gid_to_get => '( #GID#==#nan_gid# ? #nan# : #GID# * #range_step#)',
            hash       => '#get_gid#',
            makefilter =>
              '#GID#==#nan_gid# ? "#field#:-defined:1" : do { my $v= #GID# * #range_step#; Filter->newadd(1, "#field#:-<:".$v, "#field#:<:".($v + #range_step#)); }'
            ,    #FIXME decimal separator must be always "."
            'n_sort:gid' =>
              '( do{my $n=#GID#==#nan_gid# ? "-inf" : #GID# * #range_step#;warn "#GID# => $n";$n })',
            'n_sort:gid' =>
              '( #GID#==#nan_gid# ? "-inf" : #GID# * #range_step# )',
            'n_sort:gid' => '#GID#'
            , #  #nan_gid# is already the most negative number, no need to replace it with -inf
        },
        'length' => {
            ...
            'filter_prep:<' => \&::ConvertTimeLength,
            'filter_prep:b' => sub {
                sort { $a <=> $b } map ::ConvertTimeLength($_), split / /,
                  $_[0], 2;
            },
        },
        'length.div' => {
            gid_to_display =>
              'my $v=#GID# * #ARG0#; sprintf("%d:%02d", $v/60, $v%60);',
        },
        size => {
        ...
        rating => {
            ...
            display  => '(#_#==255 ? "" : #_#)',
            'stats:range' =>
              'push @{#HVAL#},#_default#;  ---- AFTER: #HVAL#=do {my ($m0,$m1)=(sort {$a <=> $b} @{#HVAL#})[0,-1]; $m0==$m1 ? $m0 : "$m0 - $m1"}',
            'stats:average' =>
              'push @{#HVAL#},#_default#;  ---- AFTER: #HVAL#=do { my $s=0; $s+=$_ for @{#HVAL#}; $s/@{#HVAL#}; }',
            check =>
              '#VAL#= #VAL# =~m/^\d+$/ ? (#VAL#>100 ? 100 : #VAL#) : "";',
            set =>
              '{ my $v=#VAL#; #_default#= ($v eq "" ? $::Options{DefaultRating} : $v); #_# = ($v eq "" ? 255 : $v); }',
            ...
            update =>
              '___default_=____; my $d=pack "C",$::Options{DefaultRating}; ___default_=~s/\xff/$d/g;'
            ,    #\xff==255 # called when $::Options{DefaultRating} has changed
        ...
        date => {
            ...
            daycount => 'do { my $t=(time-( #_# ) )/86400; ($t<0)? 0 : $t}'
            ,    #for random mode
            ...
            get_gid          => '#_# ? #mktime# : 0',
            hash =>
              '(#_# ? #mktime# : 0)', #or use post-hash modification for 0 case
...
sub newadd {
    my ($class, $and, @filters) =
      @_;    #warn "Filter::newadd(@_) called from : ".join(':',caller)."\n";
    my $self = bless {}, $class;
    my %sel;
...
sub new_from_smartstring {
    ...
    while ($string =~ m/\S/) {
        ...
        $string
          =~ s/^\\(?=[-!O|\(\)])//;    #un-escape escaped negative or OR or ()
        ...
        if ($string =~ s&^(\w+(?:\|\w+)*)?(<=|>=|[:<>=~]|#+)&&) {
            ...
            if ($fields) {
                my @f = grep $_,
                  map { $Songs::Aliases{$_} || $Songs::Aliases{::superlc($_)} }
                  split(/\|/, $fields);
                if (@f) { $fields = join '|', @f }
                else {
        ...
        my @patterns;
        {
            if ($string =~ s#^(['"])##)    #pattern begins with a quote
            {
                ...
                $string
                =~ s/^(.*?)(	(?<!\\)\| |			# ends with | => more than one pattern
        ...
        s#\\([ "'|)])#$1#g
          for @patterns;    #un-escape escaped spaces, quotes, | and )
 ...
 # convert smart operator to internal operator and create filter for this level
        ...
        for my $field (split /\|/, $fields) {
            for my $pattern (@patterns) {
                ...
                if ($pattern eq '') {
                    $filter = Songs::Field_property($field,
                        'smartfilter:' . $op1 . 'empty')
                      ;    #must contain operator ':' pattern
                }
                elsif (my $found =
                    Songs::Field_property($field, 'smartfilter:' . $op1))
                {
                    if (ref $found) {
                        ...
                        my $realop =
                          @found > 1 ? $found[$casesens ? 1 : 0] : $found;
...
sub _smartstring_date_moreless {
    ...
    if ($pat =~ m/\d[smhdwMy]/) { $suffix = 'ago' }    #relative date filter
        ...
        $pat =
          ::dates_to_timestamps($pat, ($op eq '>' || $op eq '<=') ? 1 : 0);
    }
...
sub _smartstring_number {
    ...
    if (!$is_lowest_unit
        and my ($n, $u) =
        $pat =~ m#^(\d*\.?\d+)([a-zA-Z]+)$#i)    # =5m turned into 4.5m..5.5m
    {
...
sub _smartstring_date {
    ...
    if ($pat =~ m#\d#
        and ($date1, $date2) =
        $pat =~ m#^(\d*\.?\d+[smhdwMy])?(?:\.\.|-)(\d*\.?\d+[smhdwMy])?$#i
      )    # relative date filter
    {
        ...
        and my ($n, $u) =
        $pat =~ m#^(\d*\.?\d+)([smhdwMy])$#i)    # =5h turned into 4.5h..5.5h
    {
        $suffix = 'ago';
        ($date1, $date2) =
          $u eq 's' ? ('', '') : _smartstring_round_range($n, $u);
    }
...
sub add_possible_superset #indicate a possible superset filter that could be used for optimization when the result of $superset_candidate is cached
{
    ...
    return
      if $string1
      =~ m#(?:^|\x1D)\w+:-?[th]:#;    # ignores filters with head/tail filters
    for my $superset_candidate (@_) {
        ...
        push @{$self->{superset_filters}}, $string2
          if _is_subset($superset_candidate->to_array, $arrayself);
    }
...
sub _is_subset # returns true if $f2 must be a subset of $f1	#$f1 and $f2 must be in array form	#doesn't check for head/tail filters
{
    ...
    if (!ref $f1 && !ref $f2) {
        ...
        if ($op1 eq 's' || $op1 eq 'si') {
            ...
            return ($pat1 . "\x00" . $pat2)
              =~ m/^(-?\d*\.?\d+)(\w*)\x00(-?\d*\.?\d+)\2$/ && $3 > $1;
        }
        elsif ($op1 eq '<' || $op1 eq '->') {
            return ($pat1 . "\x00" . $pat2)
              =~ m/^(-?\d*\.?\d+)(\w*)\x00(-?\d*\.?\d+)\2$/ && $3 < $1;
...
sub _between_simplify #combine ranges of consecutive between filters into fewer ranges if possible
{
    ...
    s/^(\w+):(-?)b:(-?\d*\.?\d+) \3\x1D?$/"$1:$2e:$3"/e
      for @strings;    #replace :b:5 5 by :e:5
    return @strings;
}
...
sub are_equal          #FIXME could try harder
{
    ...
    my $f2 = $_[1];
    ($f1, my $s1) =
      defined $f1 ? ref $f1 ? ($f1->{string}, $f1->{source}) : $f1 : '';
    ($f2, my $s2) =
      defined $f2 ? ref $f2 ? ($f2->{string}, $f2->{source}) : $f2 : '';
...
sub _smart_simplify    #only called for ~ filters
{
    ...
    if ($returnlist || @pats == 1) {
        ...
        return "(|\x1D"
          . join('', map("$field:$inv~:$_\x1D", @pats)) . ")\x1D";
    }
}
...
sub filter {
    ...
    if (   $on_library
        && !$self->{nocache}
        && (%CachedList || %IdleFilter::InProgress))
    {
        my $string = $self->{string};
        $CachedTime{$string} = time
          ; # the result will be cached if it is not already => update timestamp now
        return [unpack 'L*', $CachedList{$string}]
          if defined $CachedList{$string};
...
sub simplify_with_superset_cache {
    ...
    my @supersets = grep defined, map $CachedList{$_},
      @{$self->{superset_filters}};
    if (@supersets)
...
# no cached result for superset, look if there is an idlefilter in progress that could be used
    my ($idlefilter) = sort {
      ...
      grep defined, map $IdleFilter::InProgress{$_},
      @{$self->{superset_filters}};
...
sub cache_result {
    ...
    if ($CachedTotal > 100)    # trim the cache
    {
        my $time      = time;
        my @del_order = sort {
            ($time - $CachedTime{$b}) * $CachedSize{$b}
              <=> ($time - $CachedTime{$a}) * $CachedSize{$a}
        } keys %CachedSize;
...
#warn "removing ".do {my $f=$delete; $f=~s/\x1D+//g; $f}." last used=".localtime($CachedTime{$delete})." size=".$CachedSize{$delete}." score=".(($time-$CachedTime{$delete})*$CachedSize{$delete})."\n";
    ...
    $CachedTotal += $CachedSize{$string} =
      1 + (39 * @$result / (@$::Library || 1));
...
{
    my ($self, $IDs, $fields, $songarray) =
      @_;              #$songarray argument is optional, currently untested
    $self->makesub unless $self->{'sub'};
    ...
    for my $i (0 .. $#filter) {
        ...
        if ($s =~ m/^\(/) { $d++; $or[$d] = ($s eq '(|') ? 1 : 0; }
            ...
            while (my ($icc, $h) = each %$vd) {
                ...
                next unless $ok;
                if ($prephash) {
                    $prephash = Songs::Compile('filter_prephash:' . $cmd,
                        "sub {$prephash}")
                      unless ref $prephash;
                    $h = $prephash->($h);
                }
                ...
                $filter[$_] = undef
                  for @$l;    #remove filters to be replaced by the hash
                push @$hashes, $h;
            ...
            push @{$ilist[$d]{"$1:$2$3:"}},
              $i;    #store filter index to remove it later if it is replaced
        }
    }
    ...
    for my $f (@filter) {
        if ($f =~ m/^[\(\)]/) {
            ...
            if (my $sub = $NGrepSubs{$cmd}) {
                $func
                  .= $NGrepSubs{$cmd}->($field, $pat, $listref, $in[$d], $inv);
            }
            else {
...
sub _explain_element {
    ...
    my ($field, $inv, $cmd, $pattern) =
      $filter_element =~ m/^(\w+):(-?)([^:]+):(.*)$/;
    return unless $cmd;
    my $text = Songs::Field_property($field, "filterdesc:$inv$cmd:$pattern")
      || Songs::Field_property($field, "filterdesc:$inv$cmd");
...
sub SmartTitleSimplify {
    my $s = $_[0];
    $s =~ s#(?<=.) *[\(\[].*##
      ;    #remove '(...' unless '(' is at the begining of the string
...
sub QuoteRegEx {
    local $_ = $_[0];
    s#((?:\G|[^\\])(?:\\\\)*)([\$@](?:::?)?[0-9a-z])#$1\\$2#gi
      ;               #quote variable names if not already quoted
    s#^((?:.*[^\\])?(?:\\\\)*\\)$#$1\\#g
      ;               ##escape trailing '\' in impair number
    s!((?:\G|[^\\])(?:\\\\)*)\\?"!$1\\"!g
      ; #make sure " are escaped (and only once, so that \\\" doesn't become \\\\")
    if (!eval { qr/$_/; }) {
...
sub smartstring_fuzzy {
    ...
    my $threshold =
      $pat =~ s/([<>])(\d?\d)$//
      ? ($1 eq '>' ? $2 : int(100 - 100 * $2 / length($pat)) - 1)
      : $op eq '#'  ? 80
      : $op eq '##' ? 70
      :               60;
    ...
    for my $j (1 .. length $string2) {
        ...
        {    #if (substr($string1,$i-1,1) eq substr($string2,$j-1,1))
            if (   substr($string1, $i - 1, 1) eq substr($string2, $j - 1, 1)
                ...
                || substr($string2, $j - 1, 1) eq
                "'")    # treat single quote as equal to any character
            {
                ...
                my $del = $mat[$i - 1][$j] + 1;
                my $ins =
                  ($i == $length1) ? $mat[$i][$j - 1] : $mat[$i][$j - 1] + 1;
...
sub new {
    ...
    my $self =
      bless {filter => $filter, callback => $callback, started => time},
      $class;
    $self->start;
    return $self;
}
...
sub get_progress {
    ...
    unless ($progress) {
        ...
        my $todo;
        $todo = $filter->simplify_with_superset_cache
          if $filter->{superset_filters};
...
sub make {
    ...
    for my $s (split /\x1D/, $self->{string}) {
        ...
        if (my $value = $ScoreTypes{$type}{value}) {
            ...
            $depends{$_} = undef
              for Songs::Depends($ScoreTypes{$type}{depend});
        }
        else {
        ...
        $self->{time_computed} = 1
          if $ScoreTypes{$type}{time_dependant}
          ;    #indicate that the values must be recomputed as time pass
        if ($type eq 'f' || $type eq 'g') {    #$score=&$score($extra);
        }
        elsif ($type eq 'r') {
            my @l = split /,/, $extra;
            next unless @l == 11;
    ...
    $self->{before}  = '';
    if ($self->{hashes}) {
        $self->{before} .= "my \$hash$_=\$self->{hashes}[$_];"
          for 0 .. $#{$self->{hashes}};
    }
    $self->{score} = "\n(" . join(")\n*(", @scores) . ')';
    ::setlocale(::LC_NUMERIC, '');
...
sub MakeGroupScoreFunction {
    my ($self, $field) = @_;
    my ($keycode, $multi) =
      Songs::LookupCode($field, 'hash', 'hashm', [ID => '$_']);
    unless ($keycode || $multi) {
        warn
          "MakeGroupScoreFunction error : can't find code for field $field\n";
        return;
    }    #return dummy sub ?
...
sub Draw {
    ...
    if (!$self->{valid}
        || ($self->{time_computed} && time - $self->{time_computed} > 10 * 60))
    {
...
#warn "no more chunks : savedr=$savedr r=$r chunk=$chunk" if $chunk>$#chunksum;
    ...
    if ($nb && @list)    #if still need more -> select at random (no weights)
    {
        ...
        push @drawn, map $list[$_],
          (sort { $rand[$a] <=> $rand[$b] } 0 .. $#list)[0 .. $nb - 1];
    }
    return @drawn;
}
