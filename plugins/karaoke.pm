...
sub new {
    ...
    $self->modify_font(Gtk2::Pango::FontDescription->from_string($opt->{font}))
      if $opt->{font};
    $self->{attr}        = $opt->{attr};
...
sub SongChanged {
    ...
    my $file =
      ::pathfilefromformat($self->{ID}, $::Options{OPT . 'PathFile'}, undef,
        1);
    if ($file && -r $file) {
...
sub load_file {
    ...
    for my $l (split /\012|\015|\015\012/, $text) {
        ...
        {
            my $i =
              0;  #loop for cases : [mm:ss.xx][mm:ss.xx][mm:ss.xx]repeated line
    ...
    {
        @$lines = map $lines->[$_],
          sort { $seconds->[$a] <=> $seconds->[$b] } 0 .. $#$lines;
...
sub TimeChanged {
    ...
    { # $line : <mm:ss.xx> word1 <mm:ss.xx> word2 <mm:ss.xx> ... lastword <mm:ss.xx>
        my @words =
          (0, 0, 0, split /<(\d?\d):(\d\d)(?:\.(\d\d)\d?)?>/, $line, -1);
        $self->{lasttime} = -1;
        $line = '';
        my $prev = '';
        my $found;
    ...
    if (my $n = $self->{before}) {
        ...
        $text = ::PangoEsc($text);
        $text = "<span $self->{before_attr}>$text</span>"
          if $self->{before_attr};
        $line = $text . "\n" . $line;
    }
    if (my $n = $self->{after}) {
        ...
        $text =~ s/\s*<\d?\d:\d\d(?:\.\d\d\d?)?>\s*/ /g;
        $text = ::PangoEsc($text);
        $text = "<span $self->{after_attr}>$text</span>"
          if $self->{after_attr};
        $line .= "\n" . $text;
    }
