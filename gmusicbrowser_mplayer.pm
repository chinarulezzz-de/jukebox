...
sub init {
    ...
    if (   $mplayer
        && !-x $mplayer
        && !(::first {-x $_} map $_ . ::SLASH . $mplayer, split /:/,
            $ENV{PATH}))
    {
        $mplayer = undef;
    }
    $mplayer ||= ::first {-x $_} map $_ . ::SLASH . 'mplayer', split /:/,
      $ENV{PATH};
...
sub VolInit {    # check if support -volume option
...
#-hr-mp3-seek to fix wrong time with some mp3s
    ...
    push @cmd_and_args, split / /, $::Options{mplayeroptions}
      if $::Options{mplayeroptions};
    warn "@cmd_and_args\n" if $::debug;
    ...
    if (!defined $ChildPID) {
        warn "gmusicbrowser_mplayer : fork failed : $!\n";
...
sub AdvancedOptions {
    ...
    $vbox->pack_start(Play_amixer::make_option_widget(), ::FALSE, ::FALSE, 2)
      unless $SoftVolume;
    return $vbox;
}
...
sub SetVolume {
    ...
    my $vol =
      convertvolume($::Volume); #use a cubic volume scale and apply $GainFactor
...
sub RG_set_options {
    ...
    if (defined($::PlayingID) && $::Options{use_replaygain}) {
        my ($gain1, $gain2, $peak1, $peak2) = Songs::Get($::PlayingID,
            qw/replaygain_track_gain replaygain_album_gain replaygain_track_peak replaygain_album_peak/
        );
        ($gain1, $gain2, $peak1, $peak2) = ($gain2, $gain1, $peak1, $peak2)
          if $::Options{rg_albummode};
        ...
        my $invpeak = 1 / $peak;
        warn
          "gain=$gain peak=$peak => scale factor=min($GainFactor,$invpeak)\n"
          if $::debug;
        $GainFactor = $invpeak
          if $invpeak < $GainFactor;  #clipping prevention, make it an option ?
    }
    else { $GainFactor = 1 }
    return unless $ChildPID;
