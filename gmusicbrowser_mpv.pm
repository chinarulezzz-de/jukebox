...
sub init {
    ...
    if (!check_version(0, 7)) {
        $mpv = undef;
        warn
          "mpv version earlier than 0.7 are not supported -> mpv backend disabled\n";
    }
    return unless $mpv;
...
sub launch_mpv {
    ...
    @cmd_and_args = (
        ...
        qw/--idle --no-video --no-input-terminal --really-quiet --gapless-audio=weak --softvol-max=100 --mute=no --no-sub-auto/
    );
    ...
    if ($::Options{use_replaygain}) {
        if (check_version(0, 28)) {
            ...
            push @cmd_and_args,
              "--replaygain-clip=" . $::Options{rg_limiter} ? 'yes' : 'no';
            push @cmd_and_args,
              "--replaygain-fallback=" . ($::Options{rg_fallback} || 0);
        }
        else { push @cmd_and_args, "--af-add=" . get_RG_string(); }
    }
    push @cmd_and_args, "--af-add=" . get_EQ_string($::Options{equalizer})
      if $::Options{use_equalizer};
    push @cmd_and_args, split / /, $::Options{mpvoptions}
      if $::Options{mpvoptions};
    warn "@cmd_and_args\n" if $::debug;
    $ChildPID = fork;
    if (!defined $ChildPID) {
    ...
    while (my $line = <$sockfh>) {
        ...
        if (my $error = $msg->{error}) {
            ...
            if ($event eq 'property-change' && $msg->{name} eq 'path') {
                ...
                last
                  if $eof
                  ; #only do eof now to catch log-message that are only sent after end-file and start-file
            }
            elsif ($mpv_file ne $gmb_file) {
            }    #ignore all other events unless file is current
            elsif ($event eq 'property-change'
                && $msg->{name} eq 'playback-time'
                && defined $msg->{data})
            {
                ::UpdateTime($msg->{data});
            }
                ...
                $eof = 1;
            } # ignore EOF signal on user-initiated track change as $mpv_file and $gmb_file aren't equal in those cases
...
sub get_EQ_string {
    ...
    if (check_version(0, 28)) {
        ...
        return
          "\@EQ:lavfi=[firequalizer=gain_entry='"
          . (join ';', @entries) . "']";
    }
...
sub get_RG_string {
    ...
    my $mode =
      $::Options{rg_albummode} ? 'replaygain-album' : 'replaygain-track';
    my $clip   = $::Options{rg_limiter} ? 'yes' : 'no';
    my $preamp = get_RG_preamp();
    my $RGstring =
      "\@RG:volume=0:$mode=$enable:replaygain-clip=$clip:replaygain-preamp=$preamp";
    return $RGstring;
}
...
sub RG_set_options {
    if (check_version(0, 28)) {
        ...
        cmd_push('set', 'replaygain-clip',
            $::Options{rg_limiter} ? 'yes' : 'no');
