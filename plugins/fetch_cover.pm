...
use constant {
    ...
    , #google checks to see if the browser can handle the "standard" image search version, instead of the "basic" version. And as of the end of 2013 the "basic" version doesn't include direct url of the images, so we need to use the "standard" version
    BING_USER_AGENT => ' '
...
my %Sites = (
    artist => {
        ...
        lastfm =>
          ['last.fm', "http://www.last.fm/music/%a/+images", \&parse_lastfm],

#discogs => ['discogs.com', "http://api.discogs.com/search?f=xml&type=artists&q=%a", \&parse_discogs],
        bing => [
            'bing',       "http://www.bing.com/images/async?q=%s",
            \&parse_bing, BING_USER_AGENT
        ],
        yahoo => [
            'yahoo', "http://images.search.yahoo.com/search/images?p=%s&o=js",
            \&parse_yahoo
        ],
        ddg => [
            "DuckDuckGo", "https://duckduckgo.com/?q=%s&iax=1&ia=images",
            \&parse_ddg
        ],
    },
    album => {
...
sub Fetch {
    ...
    $self->{table}    = my $table =
      Gtk2::Table->new(RES_LINES, RES_PER_LINE, ::TRUE);
    $self->add(
        ::Vpack(
            [map({ ('_', $_) } @entry), $Bsearch, $Bstop, $source], '_',
            $table,                                                 '-',
    ...
    $Bcur->signal_connect(
        clicked => sub { $_[0]->get_toplevel->SearchID(undef, $::SongID) });
    $self->signal_connect(destroy => \&abort);
    $self->signal_connect(unrealize =>
          sub { $::Options{OPT . 'winsize'} = join ' ', $_[0]->get_size; });

    my $size = $::Options{OPT . 'winsize'}
      || RES_PER_LINE * PREVIEW_SIZE . ' ' . RES_LINES * PREVIEW_SIZE;
...
sub NewSearch {
    ...
    $self->{url} = $url;
    $self->{searchcontext} =
      {};    #hash that the parser can use to store data between searches
    ...
    while ($result =~ m#a title="\[Album(\d+)\]"#g) {
        push @list,
          { url => "http://static.rateyourmusic.com/album_images/$1.jpg",
            previewurl =>
              "http://static.rateyourmusic.com/album_images/s$1.jpg",
          };
    }
    return \@list;
}
...
sub parse_freecovers #FIXME could use a XML module	#can provide backcover and more too
{
    ...
    while ($result =~ m#<title>(.+?)</title>#gs) {
        my $res = $1;
        my %res;
        $res{desc} = ::decode_html(Encode::decode('cp1252', $1))
          if $res =~ m#<name>([^<]+)</name>#;   #FIXME not sure of the encoding
...
sub parse_lastfm {
    ...
    my @list;
    while ($results
        =~ m#<a\s+href="/music/[^/]+/\+images/[0-9A-F]+"[^>]+?class="image-list-link"[^<]+<img[^>]+?src="([^"]+)"#gis
      )
    {
        my $url = my $pre = $1;
    ...
    my $nexturl;
    $nexturl = $searchcontext->{baseurl} . $1
      if $results =~ m#<a href="(\?page=\d+)">Next</a>#;
    return \@list, $nexturl;
}
...
sub parse_sloth {
    ...
    $nexturl = 'http://www.slothradio.com/covers/' . $1
      if $result
      =~ m#<div class="pages">[^<>]*(?:\s*<a href="[^"]+">\d+</a>)*\s*\d\s+<a href="([^"]+)">\d+</a>#;
    return \@list, $nexturl;
}
...
sub parse_googlei {
    ...
    for my $res (split /<div class="rg_meta[^"]*"[^>]*>/, $result) {
        ...
        my $desc =
          $res =~ m/"pt":"([^"]+)"/ ? Encode::decode('utf8', $1) : undef;
    ...
    my $nexturl =
      $searchcontext->{baseurl} . "&ijn=" . $searchcontext->{pagecount};
...
sub parse_bing {
    ...
    my $nexturl =
      $searchcontext->{baseurl} . "&first=" . (1 + $n * 100) . "&count=100";
    $nexturl = undef unless @list;
    return \@list, $nexturl;
}
...
sub parse_yahoo {
    ...
    while ($result
        =~ m/<li class="ld ?"([^>]+)><a +(?:target="[^"]*" +)?href=([^>]+)>(?:<img src=['"]([^'"]+)['"])?/g
      )
    {
        my $href    = $2;
        my $preview = $3;
...
# for some reason next pages return some previous results, use $searchcontext->{seen} to ignore them
    my $seen = $searchcontext->{seen} ||= {};
    $nexturl = 'https://duckduckgo.com/' . $1
      if $result =~ m#"next"\s*:\s*"(i.js[^"]+)#;
...
sub parse_discogs {
    ...
    while ($result =~ m#<thumb>([^<]+?)/(A|R)-(\d+)-([^<]+?)</thumb>#g) {
        my $referer =
          $2 eq 'R' ? "http://www.discogs.com/viewimages?release=$3" : "";
        push @list,
          { url        => "$1/$2-$4",
            previewurl => "$1/$2-$3-$4",
            referer    => $referer,
...
sub get_next {
    ...
    if ($#$results < $end && $self->{nexturl}) {    #load next page
    ...
    $result->{waiting} = Simple_http::get_with_cb(
        ...
        cb      => sub {
            ...
            my $loader;
            $loader = GMB::Picture::LoadPixData($pixdata, PREVIEW_SIZE)
              if $pixdata;
            if ($loader) {
                ...
                $button->signal_connect(
                    button_press_event =>
                      \&GMB::Picture::pixbox_button_press_cb,
                    3
                );    # 3 : mouse button 3
                ::set_drag($button,
                    source =>
                      [::DRAG_FILE, sub { return ::DRAG_FILE, $_[0]{url} }]);
                $button->set_relief('none');
                $button->show_all;
                my $i = $res_id % RES_PER_PAGE;
                my $y = int($i / RES_PER_LINE);
                my $x = $i % RES_PER_LINE;
                $table->attach($button, $x, $x + 1, $y, $y + 1, 'fill',
                    'fill', 1, 1);
                $result->{done} = 1;
                $self->{loaded}++;
            }
            ...
            $self->{progress}->set_fraction($self->{loaded}
                  / (@{$self->{results}} - RES_PER_PAGE * $self->{page}));
            ::IdleDo('8_FetchCovers' . $self, 100, \&get_next, $self);
        }
    );
...
sub set_cover {
    ...
    $check->set_active(1);
    my $default_file =
      $::Options{OPT . 'USEFILE'} ? $::Options{OPT . 'COVERFILE'} : $name;
    ...
    $default_dir = $self->{dir}
      unless $::Options{OPT . 'USEPATH'} && -d $default_dir;
